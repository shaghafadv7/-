...
export default function Projects() {
  ...
  return (
    <>
      ...
      {/* سجل الوحدة */}
      <Modal open={!!unitView} onClose={() => setUnitView(null)} title={`سجل الوحدة ${unitView?.code}`} w={600}>
        {unitView && <div className="space-y-4">
          <div className="rounded-xl p-3 text-[13px]" style={{ background: 'var(--card2)' }}>
            {unitView.building_name || ''} {unitView.floor_name ? `— ${unitView.floor_name}` : ''} — {unitView.rooms} غرف — {fmtN(unitView.area)} م² — <b className="num" style={{ color: 'var(--brand)' }}>{fmtN(unitView.price)} {cur}</b>
          </div>
