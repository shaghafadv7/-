...
app.post('/api/files/upload', auth, P('files', 'create'), (req, res, next) => {
  ...
  const { category = 'general', tags = '[]', client_id, task_id, appointment_id, project_id, unit_id, sale_id, reservation_id } = req.body || {};
  const num = (v) => /^\d+$/.test(String(v || '')) ? Number(v) : null;
  const ins = db.prepare('INSERT INTO files (name, original_name, stored_name, mime, size, category, tags, client_id, task_id, appointment_id, project_id, unit_id, sale_id, reservation_id, uploaded_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
  const out = (req.files || []).map(f => {
    const info = ins.run(f.originalname, f.originalname, path.relative(FILES_DIR, f.path), f.mimetype, f.size, category, tags,
      num(client_id), num(task_id), num(appointment_id), num(project_id), num(unit_id), num(sale_id), num(reservation_id), req.user.id);
