...
function resource({ table, module, fields, search = [], select = null, order = 'id', enrich = null }) {
  const r = express.Router();
  r.get('/', auth, P(module, 'view'), (req, res) => {
    try {
      ...
      let where = [`${table}.deleted_at IS NULL`];
      const params = [];
      ...
      const total = db.prepare(`SELECT COUNT(*) c FROM ${table} WHERE ${where.join(' AND ')}`).get(...params).c;
      const rows = db.prepare(`${base} WHERE ${where.join(' AND ')} ORDER BY ${sortCol} ${dir} LIMIT ? OFFSET ?`).all(...params, limit, offset);
...
R.delete('/roles/:id', auth, P('roles', 'delete'), (req, res) => {
  ...
  const n = db.prepare('SELECT COUNT(*) c FROM users WHERE role_id=? AND deleted_at IS NULL').get(cur.id).c;
...
R.use('/clients', resource({
  ...
  fields: ['code', 'name', 'company', 'job_title', 'phone', 'phone2', 'email', 'address', 'city', 'category', 'status', 'source', 'notes', 'assigned_to', 'created_by'],
  search: ['clients.name', 'clients.phone', 'clients.email', 'clients.company', 'clients.code'],
  select: `SELECT clients.*, u1.name assigned_name, u2.name creator_name FROM clients LEFT JOIN users u1 ON u1.id=clients.assigned_to LEFT JOIN users u2 ON u2.id=clients.created_by`
}));
...
R.use('/units', resource({
  ...
  fields: ['code', 'project_id', 'building_id', 'floor_id', 'type', 'rooms', 'area', 'price', 'status', 'description'],
  search: ['units.code', 'units.description'],
  select: `SELECT units.*, p.code project_code, p.name project_name, b.name building_name, f.name floor_name FROM units JOIN projects p ON p.id=units.project_id LEFT JOIN buildings b ON b.id=units.building_id LEFT JOIN floors f ON f.id=units.floor_id`
}));
