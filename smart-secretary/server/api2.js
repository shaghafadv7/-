...
R.post('/sales', auth, P('sales', 'create'), (req, res) => {
  const { reservation_id, unit_id, client_id, sale_price, discount = 0, down_payment = 0, commission = 0, broker_name = '', payment_method = 'transfer', reference_no = '', sale_date, notes } = req.body || {};
  ...
  const sp = num(sale_price ?? resv?.price), dc = num(discount ?? resv?.discount), dp = num(down_payment), cm = num(commission);
  ...
  const info = db.prepare(`INSERT INTO sales (code, reservation_id, unit_id, client_id, sale_price, discount, net_price, down_payment, commission, commission_due, broker_name, settlement, payment_method, reference_no, sale_date, notes, created_by)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(code, reservation_id || null, unit.id, client.id, sp, dc, net, dp, cm, cm, broker_name, settlement, payment_method, reference_no, sale_date || today(), notes || '', req.user.id);
...
R.get('/search', auth, (req, res) => {
  ...
  if (can(p, 'units', 'view')) g('units', 'الوحدات', 'home', '/units', db.prepare('SELECT id, code title, status sub FROM units WHERE code LIKE ? LIMIT 5').all(like));
...
R.get('/reports/:name/excel', auth, P('reports', 'export'), async (req, res) => {
  ...
  def.cols.forEach((c, i) => { header.getCell(i + 1).value = COLS_AR[c] || c; header.getCell(i + 1).font = { bold: true }; });
