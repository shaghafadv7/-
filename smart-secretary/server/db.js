...
const MODULES = ['dashboard','tasks','appointments','clients','calls','notes','files','notifications','users','roles','audit','settings','reports','projects','units','reservations','sales','finance','backup','assistant'];
const ACTIONS = ['view','create','edit','delete','export','print','approve','manage'];
...
function seed() {
  ...
  grantAll(adminR);
  ['dashboard','clients','files','reports','finance','sales','reservations','assistant','notifications'].forEach(m =>
  ...
  ['dashboard','clients','projects','units','reservations','sales','calls','notes','files','reports','assistant','notifications','tasks','appointments'].forEach(m =>
  ...
  ['dashboard','tasks','appointments','clients','calls','notes','files','notifications','reports','assistant','projects','units','reservations'].forEach(m =>
  ...
  const addS = db.prepare('INSERT INTO sales (code, unit_id, client_id, sale_price, discount, net_price, down_payment, commission, commission_due, broker_name, settlement, payment_method, reference_no, sale_date, status, created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
  const s1 = addS.run('S-2026-001', availUnits[1].id, cids[1], availUnits[1].price, 10000, net, 50000, net * 0.05, net * 0.05, 'مكتب الوساطة الذهبية', net - net * 0.05, 'transfer', 'TRX-77001', d(-10), 'active', uAcc).lastInsertRowid;
...
module.exports = { db, init, flush, MODULES, ACTIONS, DATA_DIR, DB_PATH };
