...
export default function Reservations() {
  ...
  const load = () => {
    setBusy(true); setErr('');
    api('/reservations' + q({ page, limit: 12, q: search, status: fStatus }))
...
export function SaleFromReserve({ open, resv, cur, onClose, onDone }) {
  ...
  useEffect(() => { if (open && resv) setF({ reservation_id: resv.id, sale_price: resv.price, discount: resv.discount || 0, down_payment: resv.deposit || 0, commission: 0, broker_name: '', payment_method: 'transfer', reference_no: '', sale_date: new Date().toISOString().slice(0, 10), notes: '' }); }, [open]);
    ...
    <Modal open={open} onClose={onClose} title={`إتمام بيع — حجز ${resv?.code}`} w={600}
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Field label="عمولة الوسيط"><input type="number" min={0} className="inp num" value={f.commission || 0} onChange={e => S('commission', +e.target.value)} /></Field>
          <Field label="التسوية (صافٍ − عمولة)"><input className="inp num" disabled value={fmtN(settle)} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Field label="اسم الوسيط"><input className="inp" value={f.broker_name || ''} onChange={e => S('broker_name', e.target.value)} /></Field>
