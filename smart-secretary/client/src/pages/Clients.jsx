...
export default function Clients() {
  ...
  const openProfile = async (c) => {
    setProfile(c);
    try { const r = await api(`/clients/${c.id}/timeline`); setTimeline(r.data || []); } catch { setTimeline([]); }
  };
  const save = async (f) => {
                ...
                {timeline.map((t, i) => (
                  <div key={i} className="flex gap-3 pb-4 relative">
                    ...
                    <span className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10" style={{ background: 'var(--bg2)', color: 'var(--brand)' }}>
                      <I n={t.kind === 'task' ? 'check' : t.kind === 'appointment' ? 'cal' : t.kind === 'call' ? 'phone' : t.kind === 'reservation' ? 'key' : t.kind === 'sale' ? 'wallet' : 'note'} s={15} />
                    </span>
                    <div className="flex-1 rounded-xl p-2.5" style={{ background: 'var(--card2)' }}>
                      <div className="text-[13px] font-bold">{t.title}</div>
                      <div className="text-[11.5px] num" style={{ color: 'var(--ink3)' }}>{fmtD(t.date)} {t.status ? `— ${t.status}` : ''}</div>
                    </div>
                  </div>
                ))}
              </div>
