...
const ROLE_DEFAULTS = {
  ...
  sales_manager: P('view_projects', 'view_units', 'view_reservations', 'add_reservation', 'edit_reservation', 'cancel_reservation',
    ...
    'view_sales_reports', 'view_financial_reports', 'view_marketer_reports', 'view_settlements', 'manage_settlements', 'create_reports', 'export_pdf', 'print_reports', 'save_pdf', 'manage_docs_archive'),
  accountant: P('view_projects', 'view_units', 'view_reservations', 'view_sales', 'add_sale', 'view_prices', 'view_discounts', 'view_payments', 'add_payment',
    ...
    'view_sales_reports', 'view_financial_reports', 'view_marketer_reports', 'view_settlements', 'manage_settlements', 'create_reports', 'export_pdf', 'print_reports', 'save_pdf'),
...
function saleFinance(s) {
  ...
  const realizedRevenue = paid; // لا يدخل المتبقي في الإيراد أو الربح قبل تحصيله
...
app.post('/api/sales', auth, perm('add_sale'), wrap((req, res) => {
  ...
  const result = db.transaction(() => {
    ...
    return { id: i.lastInsertRowid, sale_no: no, invoice_no: invoiceNo, beneficiary_type: beneficiaryType, final_price: finalPrice, discount_amount: discountAmount, is_resale: isResale };
  })();
...
app.post('/api/marketers/:id/settlement-batches', auth, perm('pay_commission'), wrap((req,res)=>{
  const result=db.transaction(()=>{
    ...
    for(const s of rows){const amount=s.commission_total-s.commission_paid;const cpno='CMP-'+new Date().getFullYear()+'-'+String(Date.now()).slice(-7)+'-'+s.id;const cp=db.prepare('INSERT INTO commission_payments(cp_no,marketer_id,sale_id,amount,pay_date,method,ref_no,notes,created_by,settlement_batch_id) VALUES(?,?,?,?,?,?,?,?,?,?)').run(cpno,marketer.id,s.id,amount,req.body.settlement_date||nowDate(),req.body.method||'cash',req.body.ref_no||'',`تصفية إجمالية ${no}${req.body.notes?' — '+req.body.notes:''}`,req.user.id,batchId);db.prepare('INSERT INTO marketer_settlement_items(batch_id,sale_id,commission_payment_id,amount) VALUES(?,?,?,?)').run(batchId,s.id,cp.lastInsertRowid,amount);recalcCommission(s.id);financialAudit(req.user,'marketer_batch_settlement','commission_payment',cp.lastInsertRowid,s.id,{commission_paid:s.commission_paid},{commission_paid:s.commission_total,batch_no:no},req.body.notes||'');}
...
app.post('/api/settlements/:id/transactions', auth, perm('manage_settlements'), wrap((req,res) => {
  ...
  const result=db.transaction(()=>{
    ...
    if(entry.party_type==='new_owner'){
      const pno='PAY-'+new Date().getFullYear()+'-'+String(Date.now()).slice(-7);
      const r=db.prepare('INSERT INTO payments(payment_no,sale_id,amount,pay_date,method,ref_no,notes,created_by) VALUES(?,?,?,?,?,?,?,?)').run(pno,report.resale_sale_id,amount,req.body.transaction_date||nowDate(),method,req.body.ref_no||'',req.body.notes||`تصفية ${report.report_no}`,req.user.id);
      ...
      const no='CMP-'+new Date().getFullYear()+'-'+String(Date.now()).slice(-7);
      const r=db.prepare('INSERT INTO commission_payments(cp_no,marketer_id,sale_id,amount,pay_date,method,ref_no,notes,created_by) VALUES(?,?,?,?,?,?,?,?,?)').run(no,s.marketer_id,report.resale_sale_id,amount,req.body.transaction_date||nowDate(),method,req.body.ref_no||'',req.body.notes||`تصفية ${report.report_no}`,req.user.id);
