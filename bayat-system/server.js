...
// الحقول المطلوبة محاسبيًا لكل طريقة دفع.
// المستوى الصارم اختياري ويُفعَّل من إعدادات النظام (strict_payment_fields=1)
// حتى لا يمنع عمليات صحيحة في المنشآت التي لا تسجّل كل الحقول.
// أما ما يُحدد هوية العملية (رقم الحوالة/الشيك) فمفروض دائمًا في validatePaymentMethod.
const PAYMENT_METHOD_FIELDS = {
  bank_transfer: ['bank', 'deposit_account'],
  check: ['bank', 'check_due_date'],
  cash: ['received_by'],
};
const PAYMENT_FIELD_AR = {
  bank: 'البنك', deposit_account: 'الحساب المستلم', ref_no: 'رقم المرجع / التحويل / الشيك',
  check_due_date: 'تاريخ استحقاق الشيك', received_by: 'اسم المستلم',
};
function strictPaymentFieldsOn() {
  const r = db.prepare("SELECT value FROM settings WHERE key='strict_payment_fields'").get();
  return r ? String(r.value) === '1' : false;
}
function requirePaymentFields(method, body) {
  if (!strictPaymentFieldsOn()) return;
  const needed = PAYMENT_METHOD_FIELDS[method] || [];
  const missing = needed.filter(k => !String(body[k] ?? '').trim());
  if (missing.length) throw Error('بيانات ناقصة لطريقة الدفع المختارة: ' + missing.map(k => PAYMENT_FIELD_AR[k]).join('، '));
}
// حالة الدفعة المسموح بها فقط
const VALID_PAYMENT_STATUSES = ['received', 'deposited', 'pending', 'refunded', 'cancelled'];
function validatePaymentStatus(v) {
  const st = String(v || 'received');
  if (!VALID_PAYMENT_STATUSES.includes(st)) throw Error('حالة الدفعة غير صحيحة');
  return st;
}
// v1.12: منع تكرار الرقم المرجعي — نفس رقم الشيك/الحوالة على نفس البنك يُرفض
// (النقدي معفى لأنه بلا مرجع بنكي). يفحص جدولي الدفعات ودفعات الحجز معًا.
function assertUniqueRef(method, refNo, bank) {
  const ref = String(refNo || '').trim();
  if (!ref || method === 'cash') return;
  const b = String(bank || '').trim();
  const dup1 = db.prepare("SELECT payment_no FROM payments WHERE status='active' AND method=? AND ref_no=? AND COALESCE(bank,'')=?").get(method, ref, b);
  if (dup1) throw Error(`الرقم المرجعي «${ref}» مسجل مسبقًا في العملية ${dup1.payment_no} على نفس البنك — يمنع التكرار`);
  const dup2 = db.prepare("SELECT payment_no FROM reservation_payments WHERE status NOT IN ('cancelled') AND method=? AND ref_no=? AND COALESCE(bank,'')=?").get(method, ref, b);
  if (dup2) throw Error(`الرقم المرجعي «${ref}» مسجل مسبقًا في قيد الحجز ${dup2.payment_no} على نفس البنك — يمنع التكرار`);
}
...
// ==================== المحرك المالي المركزي ====================
// كل رقم مالي في النظام يخرج من lib/finance.js — لا حساب محلي في أي شاشة أو تقرير.
const FIN = require('./lib/finance').build(db);
const saleDiscountsSum = FIN.discountsSum;
const saleExpensesSum = FIN.expensesSum;
const saleAdjustments = FIN.adjustments;
const saleTotalPaid = FIN.totalPaid;
const salePaidBreakdown = FIN.paidBreakdown;
const saleFinance = FIN.saleFinance;
const recalcSale = FIN.recalcSale;
const recalcCommission = FIN.recalcCommission;
...
function saleDepositInfo(saleId) {
  ...
  const counted = rows.filter(x => FIN.LINKED_DEPOSIT_STATUSES.includes(x.status) && FIN.COLLECTED_STATUSES.includes(x.payment_status || 'received'));
  return { count: rows.length, total: counted.reduce((a, x) => a + x.amount, 0), payments: rows };
}
...
function profitDashboardData(q = {}) {
  ...
  const totals = rows.reduce((a, r) => {
    ...
    a.commissions_paid += r.commission_paid; a.commissions_due += r.commission_remaining;
    a.discounts += r.discounts; a.pending += r.pending_amount; a.client_credit += r.client_credit;
    a.gross_profit += r.gross_profit;
    a.total_costs += r.total_costs; a.net_profit += r.net_profit; return a;
  }, { sales:0,collected:0,receivables:0,property_cost:0,expenses:0,commissions:0,commissions_paid:0,commissions_due:0,discounts:0,pending:0,client_credit:0,gross_profit:0,total_costs:0,net_profit:0 });
...
app.put('/api/settings', auth, perm('edit_branding'), wrap((req, res) => {
  ...
  tx();
  // v1.12: سياسة مالية — تتطلب صلاحية إدارة الإعدادات لا مجرد تعديل الهوية
  if (req.body.strict_payment_fields != null) {
    if (!hasPerm(req.user, 'manage_settings')) return res.status(403).json({ error: 'تغيير سياسة الحقول المحاسبية يتطلب صلاحية «إدارة إعدادات النظام»' });
    const v = String(req.body.strict_payment_fields) === '1' ? '1' : '0';
    q.run('strict_payment_fields', v);
    log(req.user, 'update', 'settings', 'strict_payment_fields', { value: v });
  }
  log(req.user, 'update', 'settings', 'brand', req.body); res.json({ ok: true });
}));
...
app.get('/api/customer-finance', auth, customerFinancePerm, wrap((req,res) => {
  // فلاتر حساب العميل: العميل / المشروع / العقار / حالة الصفقة / الفترة / حالة السداد
  const q=req.query, where=["s.status!='cancelled'"], p=[];
  if(q.customer_id){where.push('s.customer_id=?');p.push(+q.customer_id);}
  if(q.project_id){where.push('u.project_id=?');p.push(+q.project_id);}
  if(q.unit_id){where.push('s.unit_id=?');p.push(+q.unit_id);}
  if(q.unit_number){where.push('u.unit_number LIKE ?');p.push(`%${q.unit_number}%`);}
  if(q.sale_status){where.push('s.status=?');p.push(q.sale_status);}
  if(q.date_from){where.push('s.sale_date>=?');p.push(q.date_from);}
  if(q.date_to){where.push('s.sale_date<=?');p.push(q.date_to);}
  if(q.outstanding==='1') where.push('s.remaining_amount>0.005');
  if(q.search){where.push('(c.name LIKE ? OR c.phone LIKE ? OR s.sale_no LIKE ? OR s.invoice_no LIKE ? OR u.unit_number LIKE ?)');for(let i=0;i<5;i++)p.push(`%${q.search}%`);}
  const rows=db.prepare(`SELECT c.id customer_id,c.name customer_name,c.phone,c.email,s.id sale_id,s.sale_no,s.invoice_no,s.sale_date,s.is_resale,s.status sale_status,
    u.unit_number,u.id unit_id,pr.name project_name,pr.id project_id,
    ...
    WHERE ${where.join(' AND ')} ORDER BY s.sale_date DESC,s.id DESC LIMIT 1000`).all(...p)
    // كل الأرقام المالية من المحرك المركزي — لا حساب مستقل هنا
    .map(r=>{const f=saleFinance(r.sale_id);
      return {...r,final_price:f.final,paid_amount:f.paid,remaining_amount:f.remaining,discount_amount:f.discounts,
        client_credit:f.client_credit,finance:f,
        settlement_status:f.client_state==='settled'?'settled':(f.client_state==='due_to_client'?'overpaid':(f.paid>0?'partial':'unpaid'))};});
  // فلتر حالة السداد يُطبق بعد الحساب الموحد (يعتمد على أرقام المحرك)
  const filtered = q.settlement_status ? rows.filter(r=>r.settlement_status===q.settlement_status)
    : (q.status==='settled' ? rows.filter(r=>r.settlement_status==='settled') : rows);
  const totals=filtered.reduce((a,r)=>{a.sales+=r.final_price;a.paid+=r.paid_amount;a.remaining+=Math.max(0,r.remaining_amount);a.credit+=r.client_credit;a.count++;return a;},{sales:0,paid:0,remaining:0,credit:0,count:0});
  res.json({rows:filtered,totals,filters:q});
}));
app.get('/api/customer-finance/:customerId', auth, customerFinancePerm, wrap((req,res) => {
  ...
  // ملخص العميل: عدد العقارات ← قيمة العقود ← المدفوع ← المتبقي (من المحرك المركزي)
  const totals=sales.reduce((a,s)=>{a.units++;a.sales+=s.finance.final;a.paid+=s.finance.paid;a.remaining+=Math.max(0,s.finance.remaining);a.credit+=s.finance.client_credit;return a;},{units:0,sales:0,paid:0,remaining:0,credit:0});
  res.json({customer,sales,movements,totals,settlement_status:totals.remaining<=0.005?(totals.credit>0.005?'overpaid':'settled'):(totals.paid>0?'partial':'unpaid')});
}));
...
app.post('/api/reservations', auth, perm('add_reservation'), idemGuard, wrap((req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    const no = uniqueNo('RSV', 'reservations', 'reservation_no');
    ...
    if (deposit > 0) {
      ...
      requirePaymentFields(pMethod, { ...b, ref_no: b.pay_ref });
      assertUniqueRef(pMethod, b.pay_ref, b.bank);
      validatePaymentStatus(b.payment_status);
...
app.get('/api/finance/payment-ledger', auth, perm('view_payments'), wrap((req, res) => {
  // فلاتر موحدة تعمل معًا في آنٍ واحد: المشروع + العميل + العقار + الطريقة + البنك +
  // الحساب المستلم + المستلم + الحالة + الفترة + نطاق المبلغ + المرجع + رقم العملية
  const q = req.query;
  const w = ['1=1'], p = [];
  const like = (col, v) => { w.push(`${col} LIKE ?`); p.push(`%${v}%`); };
  ...
  if (q.status) { w.push('ledger.status=?'); p.push(q.status); }
  if (q.project_id) { w.push('ledger.project_id=?'); p.push(+q.project_id); }
  if (q.customer_id) { w.push('ledger.customer_id=?'); p.push(+q.customer_id); }
  if (q.unit_id) { w.push('ledger.unit_id=?'); p.push(+q.unit_id); }
  if (q.unit_number) like('ledger.unit_number', q.unit_number);
  if (q.customer) like('ledger.customer_name', q.customer);
  if (q.bank) like('COALESCE(ledger.bank,\'\')', q.bank);
  if (q.deposit_account) like('COALESCE(ledger.deposit_account,\'\')', q.deposit_account);
  if (q.recipient) like('COALESCE(ledger.received_by,\'\')', q.recipient);
  if (q.ref_no) like('COALESCE(ledger.ref_no,\'\')', q.ref_no);
  if (q.payment_no) like('ledger.payment_no', q.payment_no);
  if (q.operation_no) like('ledger.operation_no', q.operation_no);
  if (q.amount_from) { w.push('ledger.amount>=?'); p.push(+q.amount_from); }
  if (q.amount_to) { w.push('ledger.amount<=?'); p.push(+q.amount_to); }
  if (q.user_id) { w.push('ledger.created_by=?'); p.push(+q.user_id); }
  // فلتر «الحساب» القديم (توافق خلفي): يبحث في الحساب والبنك والمستلم معًا
  if (q.account) { w.push('(COALESCE(ledger.deposit_account,\'\') LIKE ? OR COALESCE(ledger.bank,\'\') LIKE ? OR COALESCE(ledger.received_by,\'\') LIKE ?)'); p.push(`%${q.account}%`, `%${q.account}%`, `%${q.account}%`); }
  ...
  const sql = `SELECT * FROM (
    SELECT 'reservation' type, rp.payment_no, rp.amount, rp.pay_date, rp.created_at, rp.method, rp.ref_no, rp.bank, rp.received_by, rp.deposit_account,
      COALESCE(rp.payment_status,'received') payment_status, rp.status, rp.created_by, rp.id source_id, rp.customer_id,
      r.reservation_no operation_no, 'حجز' operation_type, c.name customer_name, u.unit_number, u.id unit_id, p.name project_name, p.id project_id,
    ...
    UNION ALL
    SELECT 'sale' type, py.payment_no, py.amount, py.pay_date, py.created_at, py.method, py.ref_no, py.bank, py.received_by, py.deposit_account,
      COALESCE(py.payment_status,'received') payment_status, py.status, py.created_by, py.id source_id, s.customer_id customer_id,
      s.sale_no operation_no, 'بيع' operation_type, c.name customer_name, u.unit_number, u.id unit_id, p.name project_name, p.id project_id,
  ...
  ) ledger WHERE ${w.join(' AND ')} ORDER BY ledger.pay_date DESC, ledger.created_at DESC, ledger.payment_no DESC LIMIT 2000`;
  const rows = db.prepare(sql).all(...p);
  // الإجماليات تتبع نفس سياسة المحرك المالي: المحصّل فقط هو تحصيل فعلي
  const COLLECTED = require('./lib/finance').COLLECTED_STATUSES;
  const totals = rows.reduce((a, x) => {
    a.count++; a.amount += x.amount;
    if (x.status === 'cancelled') a.cancelled += x.amount;
    else if (COLLECTED.includes(x.payment_status)) a.collected += x.amount;
    else if (x.payment_status === 'pending') a.pending += x.amount;
    else if (x.payment_status === 'refunded') a.refunded += x.amount;
    return a;
  }, { count: 0, amount: 0, collected: 0, pending: 0, refunded: 0, cancelled: 0 });
  res.json({ rows, totals, filters: q });
}));
...
function createSaleCore(req, b, ctx = {}) {
  return db.transaction(() => {
    ...
    const no = uniqueNo('SAL', 'sales', 'sale_no');
    ...
    if (discountAmount > 0) {
      const dno = uniqueNo('DSC', 'discounts', 'discount_no');
    ...
    if (extraPaid > 0) {
      const pno = uniqueNo('PAY', 'payments', 'payment_no');
...
app.put('/api/sales/:id', auth, perm('edit_sale'), wrap((req, res) => {
  const tx = db.transaction(() => {
    ...
    if (wantsDiscount) {
      ...
      if (!(finalPrice > 0)) throw Error('السعر النهائي بعد الخصم يجب أن يكون أكبر من صفر');
      const paidNow = saleTotalPaid(s.id); // يشمل العربون المربوط — لا يُحسب مرتين
...
app.post('/api/sales/:id/payments', auth, perm('add_payment'), idemGuard, wrap((req, res) => {
  const tx = db.transaction(() => {
    ...
    const method = validatePaymentMethod(req.body.method, req.body.ref_no);
    requirePaymentFields(method, req.body);
    const payStatus = validatePaymentStatus(req.body.payment_status);
    // v1.9: منع الدفع الزائد إلا بصلاحية خاصة (allow_overpayments)
    const fNow = saleFinance(s);
    const remainingNow = fNow.remaining;
    ...
    // منع تكرار الرقم المرجعي لنفس البنك/الطريقة (شيك أو حوالة بنفس الرقم مرتين)
    assertUniqueRef(method, req.body.ref_no, req.body.bank);
    const no = uniqueNo('PAY', 'payments', 'payment_no');
    // v1.10: اسم المستلم + الحساب/الخزينة + حالة الدفعة • v1.12: البنك وبيانات الشيك
    db.prepare(`INSERT INTO payments(payment_no,sale_id,amount,pay_date,method,ref_no,notes,created_by,received_by,deposit_account,payment_status,bank,check_due_date,check_status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .run(no, s.id, amount, req.body.pay_date || nowDate(), method, req.body.ref_no || '', req.body.notes || '', req.user.id,
        req.body.received_by || '', req.body.deposit_account || '', payStatus,
        req.body.bank || '', method === 'check' ? (req.body.check_due_date || null) : null,
        method === 'check' ? (req.body.check_status || 'pending') : null);
    recalcSale(s.id);
...
function syncSettlementReport(reportId) {
  ...
  const definitions = [
    ...
    { type: 'previous_owner', id: prev?.customer_id || null, name: s.prev_owner_name || s.seller_name || prev?.customer_name || 'المالك السابق', direction: 'payable', gross: grossAfterDebits, deductions: saleDeductions + prevRemaining + commission + expenses, net: sellerNet, sourcePaid: 0, notes: 'صافي مستحق المستثمر بعد الخصومات والتسويات ومستحق الشركة وعمولة المسوق والمصروفات' },
...
function settlementReportDetails(idOrNo) {
  ...
  // v1.12.2: تفصيل بنود الخصم من مستحق المالك السابق — يوضح «من أين جاء الرقم»
  const sale = db.prepare('SELECT * FROM sales WHERE id=?').get(row.resale_sale_id);
  const prevSale = sale.prev_sale_id ? db.prepare('SELECT * FROM sales WHERE id=?').get(sale.prev_sale_id) : null;
  const adjS = saleAdjustments(sale.id);
  row.expenses = db.prepare("SELECT expense_no,title,amount,expense_date,notes FROM expenses WHERE sale_id=? AND status='active' ORDER BY id").all(sale.id);
  row.discounts_list = db.prepare("SELECT discount_no,amount,reason,discount_date FROM discounts WHERE sale_id=? AND status='active' ORDER BY id").all(sale.id);
  row.commission_payments = db.prepare(`SELECT cp.cp_no,cp.amount,cp.pay_date,cp.status,mk.name marketer_name FROM commission_payments cp LEFT JOIN marketers mk ON mk.id=cp.marketer_id WHERE cp.sale_id=? ORDER BY cp.id`).all(sale.id);
  const expensesTotal = row.expenses.reduce((a,x)=>a+x.amount,0);
  const prevRem = Math.max(0, +(prevSale?.remaining_amount || 0));
  const commTotal = Math.max(0, +(sale.commission_total || 0));
  row.seller_breakdown = {
    final_price: +sale.final_price,
    deductions: [
      { key: 'prev_remaining', label: 'مستحق الشركة من العقد السابق', amount: prevRem, count: prevRem > 0 ? 1 : 0 },
      { key: 'commission', label: 'عمولة المسوق' + (row.marketer_name ? ' — ' + row.marketer_name : ''), amount: commTotal, count: commTotal > 0 ? 1 : 0 },
      { key: 'expenses', label: 'مصروفات الصفقة', amount: expensesTotal, count: row.expenses.length },
      { key: 'adjustments', label: 'تسويات معتمدة (تخفيض ذمة)', amount: adjS.credit, count: adjS.credit > 0 ? 1 : 0 },
    ].filter(x => x.amount > 0),
    total_deductions: prevRem + commTotal + expensesTotal,
    net_to_seller: Math.max(0, +sale.final_price - prevRem - commTotal - expensesTotal),
    formula: 'صافي مستحق المالك السابق = السعر النهائي − مستحق الشركة − عمولة المسوق − المصروفات',
  };
  return row;
}
...
function salesReportData(q = {}) {
  ...
  // الأرقام المعروضة تُحقن من المحرك المالي حتى تتطابق مع الشاشة وPDF وExcel
  const rows = db.prepare(`${SALE_OUT_SQL} WHERE ${where.join(' AND ')} ORDER BY s.sale_date DESC, s.id DESC LIMIT 1000`).all(...p)
    .map(s => { const f = saleFinance(s); return { ...s, base_price: f.base, discount_amount: f.discounts, final_price: f.final, paid_amount: f.paid, remaining_amount: f.remaining, finance: f }; });
  const t = { base: 0, discount: 0, final: 0, paid: 0, remaining: 0, count: rows.length };
  rows.forEach(s => { t.base += s.base_price; t.discount += s.discount_amount; t.final += s.final_price; t.paid += s.paid_amount; t.remaining += Math.max(0, s.remaining_amount); });
  return { rows, totals: t };
}
function paymentsReportData(q = {}) {
  // نفس فلاتر شاشة الدفعات بالضبط — لتتطابق نتيجة الشاشة مع PDF وExcel
  const where = ["py.status='active'"], p = [];
  if (q.date_from) { where.push('py.pay_date>=?'); p.push(q.date_from); }
  if (q.date_to) { where.push('py.pay_date<=?'); p.push(q.date_to); }
  if (q.method) { where.push('py.method=?'); p.push(q.method); }
  if (q.payment_status) { where.push("COALESCE(py.payment_status,'received')=?"); p.push(q.payment_status); }
  if (q.project_id) { where.push('u.project_id=?'); p.push(+q.project_id); }
  if (q.customer_id) { where.push('s.customer_id=?'); p.push(+q.customer_id); }
  if (q.unit_number) { where.push('u.unit_number LIKE ?'); p.push(`%${q.unit_number}%`); }
  if (q.bank) { where.push("COALESCE(py.bank,'') LIKE ?"); p.push(`%${q.bank}%`); }
  if (q.deposit_account) { where.push("COALESCE(py.deposit_account,'') LIKE ?"); p.push(`%${q.deposit_account}%`); }
  if (q.recipient) { where.push("COALESCE(py.received_by,'') LIKE ?"); p.push(`%${q.recipient}%`); }
  if (q.ref_no) { where.push("COALESCE(py.ref_no,'') LIKE ?"); p.push(`%${q.ref_no}%`); }
  if (q.amount_from) { where.push('py.amount>=?'); p.push(+q.amount_from); }
  if (q.amount_to) { where.push('py.amount<=?'); p.push(+q.amount_to); }
  if (q.search) { where.push('(py.payment_no LIKE ? OR s.sale_no LIKE ? OR u.unit_number LIKE ? OR c.name LIKE ?)'); p.push(`%${q.search}%`, `%${q.search}%`, `%${q.search}%`, `%${q.search}%`); }
  const rows = db.prepare(`${PAYMENT_OUT_SQL} WHERE ${where.join(' AND ')} ORDER BY py.pay_date DESC, py.id DESC LIMIT 1000`).all(...p);
  const COLLECTED = require('./lib/finance').COLLECTED_STATUSES;
  const totals = rows.reduce((a, x) => {
    a.amount += x.amount; a.count++;
    const st = x.payment_status || 'received';
    if (COLLECTED.includes(st)) a.collected += x.amount;
    else if (st === 'pending') a.pending += x.amount;
    else if (st === 'refunded') a.refunded += x.amount;
    return a;
  }, { amount: 0, count: 0, collected: 0, pending: 0, refunded: 0 });
  return { rows, totals };
}
function commissionsReportData(q = {}) {
  ...
  const rows = db.prepare(`${SALE_OUT_SQL} WHERE ${where.join(' AND ')} ORDER BY s.id DESC LIMIT 1000`).all(...p)
    .map(s => { const f = saleFinance(s); return { ...s, final_price: f.final, paid_amount: f.paid, remaining_amount: f.remaining, commission_paid: f.commission_paid, commission_due: f.commission_remaining }; });
  ...
  rows.forEach(s => { t.total += s.commission_total || 0; t.paid += s.commission_paid; t.remaining += s.commission_due; });
...
function financialReportData(q = {}) {
  // التقرير المالي الشامل للإدارة — كل الأرقام من المحرك المالي المركزي.
  // الإيرادات / المصروفات / العمولات / الأرصدة / النتيجة — كل معلومة مرة واحدة فقط.
  const pf = profitDashboardData(q);
  const summary = {
    // الإيرادات
    final_total: pf.totals.sales,              // قيمة المبيعات (العقود)
    paid_total: pf.totals.collected,           // التحصيل الفعلي
    discount_total: pf.totals.discounts,
    // المصروفات والتكاليف
    property_cost: pf.totals.property_cost,
    expenses_total: pf.totals.expenses,
    // العمولات
    commission_due: pf.totals.commissions,               // العمولات المستحقة
    commission_paid: pf.totals.commissions_paid,         // العمولات المدفوعة
    commission_remaining: pf.totals.commissions_due,     // المتبقي للمسوقين
    // الأرصدة
    remaining_total: pf.totals.receivables,    // مستحقات العملاء / المبالغ المتبقية
    client_credit_total: pf.totals.client_credit, // المبالغ الزائدة للعملاء (رصيد دائن)
    pending_total: pf.totals.pending,          // دفعات معلقة غير محتسبة
    // النتيجة
    gross_profit: pf.totals.gross_profit,
    total_costs: pf.totals.total_costs,
    net_profit: pf.totals.net_profit,
    // مؤشرات عامة
    sales_count: pf.rows.length,
  ...
  const months = {};
  const M = () => ({ month: '', sales_count: 0, final_total: 0, discount_total: 0, paid_total: 0, property_cost: 0, expenses_total: 0, commission_total: 0, total_costs: 0, net_profit: 0, marketer_paid: 0 });
  ...
  const payWhere = ["py.status='active'", "COALESCE(py.payment_status,'received') IN ('received','deposited')"], pp = [];
  if (q.date_from) { payWhere.push('py.pay_date>=?'); pp.push(q.date_from); }
  if (q.date_to) { payWhere.push('py.pay_date<=?'); pp.push(q.date_to); }
  if (q.project_id) { payWhere.push('EXISTS(SELECT 1 FROM sales s2 JOIN units u2 ON u2.id=s2.unit_id WHERE s2.id=py.sale_id AND u2.project_id=?)'); pp.push(+q.project_id); }
  ...
  const filters = [];
  if (q.date_from) filters.push(['من تاريخ', q.date_from]);
  if (q.date_to) filters.push(['إلى تاريخ', q.date_to]);
  if (q.project_id) { const pr = db.prepare('SELECT name FROM projects WHERE id=?').get(+q.project_id); if (pr) filters.push(['المشروع', pr.name]); }
  return { summary, months: Object.values(months).sort((a, b) => b.month.localeCompare(a.month)), filters, formula: pf.formula };
}

// ===== v1.12: حساب المشروع — ملخص إداري مختصر ثم جدول العمليات =====
// فلاتر: المشروع + الفترة (وتُعرض الدفعات والمصروفات والعمولات وصافي الربح)
function projectAccountData(q = {}) {
  const projectId = q.project_id ? +q.project_id : null;
  const pf = profitDashboardData({ project_id: projectId, date_from: q.date_from, date_to: q.date_to });
  // إحصاء الوحدات للمشروع (أو كل المشاريع)
  const unitWhere = projectId ? 'WHERE u.project_id=?' : '';
  const unitArgs = projectId ? [projectId] : [];
  const unitStats = db.prepare(`SELECT
      COUNT(*) total,
      SUM(CASE WHEN u.status IN ('sold','paid') THEN 1 ELSE 0 END) sold,
      SUM(CASE WHEN u.status='reserved' THEN 1 ELSE 0 END) reserved,
      SUM(CASE WHEN u.status IN ('available','resale') THEN 1 ELSE 0 END) available
    FROM units u ${unitWhere}`).get(...unitArgs);
  const summary = {
    units_total: unitStats.total || 0, units_sold: unitStats.sold || 0,
    units_reserved: unitStats.reserved || 0, units_available: unitStats.available || 0,
    sales_value: pf.totals.sales,             // قيمة المبيعات
    collected: pf.totals.collected,           // إجمالي التحصيل
    remaining: pf.totals.receivables,         // المتبقي
    expenses: pf.totals.expenses,             // إجمالي المصروفات
    commissions: pf.totals.commissions,       // إجمالي العمولات
    commissions_paid: pf.totals.commissions_paid,
    commissions_due: pf.totals.commissions_due,
    property_cost: pf.totals.property_cost,
    client_credit: pf.totals.client_credit,
    net_profit: pf.totals.net_profit,         // صافي الربح
  };
  // جدول العمليات — صف واحد لكل صفقة بأرقام المحرك (بلا تكرار)
  const rows = pf.rows.map(r => ({
    sale_id: r.id, sale_no: r.sale_no, invoice_no: r.invoice_no, sale_date: r.sale_date,
    unit_number: r.unit_number, project_name: r.project_name, customer_name: r.customer_name,
    is_resale: !!r.is_resale, status: r.status,
    base: r.base, discounts: r.discounts, final: r.final, paid: r.paid,
    remaining: r.remaining, commission: r.commission, expenses: r.expenses, net_profit: r.net_profit,
  }));
  const project = projectId ? db.prepare('SELECT id,code,name FROM projects WHERE id=?').get(projectId) : null;
  const filters = [];
  if (project) filters.push(['المشروع', project.name]);
  if (q.date_from || q.date_to) filters.push(['الفترة', `${q.date_from || 'البداية'} ← ${q.date_to || 'اليوم'}`]);
  return { project, summary, rows, totals: summary, filters };
}
app.get('/api/reports/project-account', auth, perm('view_financial_reports'), wrap((req, res) => res.json(projectAccountData(req.query))));
...
app.get('/api/finance/integrity', auth, perm('view_financial_reports'), wrap((req, res) => {
  // فحص السلامة المالية المركزي — يقارن المخزَّن بما يحسبه المحرك المالي
  // ويعرض لكل اختلاف: الصفقة، المرجع، المشكلة، القيمة الحالية، القيمة الصحيحة، والسبب.
  ...
  for (const s of sales) {
    const f = saleFinance(s);
    const ref = { sale_id: s.id, sale_no: s.sale_no, invoice_no: s.invoice_no || null };
    if (!near(f.paid, s.paid_amount))
      add({ ...ref, type: 'paid_mismatch', problem: 'إجمالي المدفوع المخزَّن لا يطابق مجموع الدفعات الفعلية', current: s.paid_amount, expected: f.paid, reason: `دفعات نشطة ${f.paid_payments} + عربون مربوط ${f.paid_deposit}` });
    if (!near(f.final, s.final_price))
      add({ ...ref, type: 'final_mismatch', problem: 'السعر النهائي المخزَّن لا يطابق المعادلة', current: s.final_price, expected: f.final, reason: `الأساس ${f.base} − خصومات ${f.discounts} + تسويات ${f.adjustments.net}` });
    if (!near(f.remaining, s.remaining_amount))
      add({ ...ref, type: 'remaining_mismatch', problem: 'المتبقي المخزَّن لا يطابق (النهائي − المدفوع)', current: s.remaining_amount, expected: f.remaining, reason: `${f.final} − ${f.paid}` });
    if (!near(f.discounts, s.discount_amount))
      add({ ...ref, type: 'discount_mismatch', problem: 'قيمة الخصم المخزَّنة لا تطابق بنود الخصم النشطة', current: s.discount_amount, expected: f.discounts, reason: 'مجموع جدول discounts بحالة active' });
    if (!near(f.commission_paid, s.commission_paid))
      add({ ...ref, type: 'commission_paid_mismatch', problem: 'المدفوع من العمولة لا يطابق دفعات العمولة النشطة', current: s.commission_paid, expected: f.commission_paid, reason: 'مجموع commission_payments بحالة active' });
    if ((s.commission_total || 0) > 0 && f.commission_paid > (s.commission_total || 0) + 0.01)
      add({ ...ref, type: 'commission_overpaid', problem: 'المدفوع للمسوق يتجاوز العمولة المستحقة', current: f.commission_paid, expected: s.commission_total, reason: 'تصفية عمولة زائدة أو مكررة' });
    if (f.overpaid > 0.01)
      add({ ...ref, type: 'client_credit', problem: 'دفع زائد عن قيمة الصفقة — رصيد دائن للعميل (لا يدخل في الأرباح)', current: f.paid, expected: f.final, reason: `رصيد للعميل ${f.overpaid}`, severity: 'info' });
    if (f.pending_amount > 0.01)
      add({ ...ref, type: 'pending_payments', problem: 'توجد دفعات معلقة غير محتسبة في التحصيل', current: f.pending_amount, expected: 0, reason: 'حالة الدفعة = معلقة', severity: 'info' });
    // العربون: يجب ألا يُحتسب مرتين ولا يبقى نشطًا بعد الربط بالبيع
    const looseDeposit = db.prepare("SELECT payment_no,amount FROM reservation_payments WHERE sale_id=? AND status='active'").all(s.id);
    looseDeposit.forEach(d => add({ ...ref, type: 'deposit_not_linked', reference: d.payment_no, problem: 'عربون مرتبط بالبيع لكن حالته لم تُنقل إلى transferred', current: 'active', expected: 'transferred', reason: 'لن يُحتسب ضمن المدفوع حتى تُصحح الحالة' }));
    const multiRes = db.prepare("SELECT COUNT(DISTINCT reservation_id) n FROM reservation_payments WHERE sale_id=?").get(s.id).n;
    if (multiRes > 1) add({ ...ref, type: 'deposit_multi_reservation', problem: 'عربونات من أكثر من حجز مربوطة بنفس البيع', current: multiRes, expected: 1, reason: 'يحتمل احتساب العربون مرتين' });
  }

  // بيعان/حجزان نشطان لنفس الوحدة
  db.prepare("SELECT unit_id,COUNT(*) n FROM sales WHERE status='active' GROUP BY unit_id HAVING n>1").all()
    .forEach(x => add({ type: 'duplicate_active_sale', unit_id: x.unit_id, problem: 'أكثر من بيع نشط لنفس الوحدة', current: x.n, expected: 1, reason: 'تعارض في دورة البيع' }));
  db.prepare("SELECT unit_id,COUNT(*) n FROM reservations WHERE status='active' GROUP BY unit_id HAVING n>1").all()
    .forEach(x => add({ type: 'duplicate_active_reservation', unit_id: x.unit_id, problem: 'أكثر من حجز نشط لنفس الوحدة', current: x.n, expected: 1, reason: 'تعارض في دورة الحجز' }));
  db.prepare(`SELECT r.id reservation_id,r.reservation_no,u.unit_number FROM reservations r JOIN units u ON u.id=r.unit_id
    WHERE r.status='active' AND EXISTS(SELECT 1 FROM sales s WHERE s.unit_id=r.unit_id AND s.status='active')`).all()
    .forEach(c => add({ type: 'active_reservation_with_sale', ...c, problem: 'حجز نشط على وحدة مباعة فعليًا', current: 'active', expected: 'converted/cancelled', reason: 'الوحدة عليها بيع نشط' }));
  // أرقام مرجعية مكررة (شيك/حوالة على نفس البنك)
  db.prepare(`SELECT method,ref_no,COALESCE(bank,'') bank,COUNT(*) n FROM payments WHERE status='active' AND method!='cash' AND COALESCE(ref_no,'')!=''
    GROUP BY method,ref_no,COALESCE(bank,'') HAVING n>1`).all()
    .forEach(x => add({ type: 'duplicate_ref_no', reference: x.ref_no, problem: 'رقم مرجعي مكرر على نفس البنك والطريقة', current: x.n, expected: 1, reason: `${x.method} — ${x.bank || 'بدون بنك'}` }));
  // أرصدة المشاريع: مجموع الصفقات لكل مشروع
  const projects = db.prepare(`SELECT p.id,p.name,COUNT(s.id) sales_count FROM projects p
    LEFT JOIN units u ON u.project_id=p.id LEFT JOIN sales s ON s.unit_id=u.id AND s.status!='cancelled'
    WHERE p.active=1 GROUP BY p.id`).all();

  const errors = issues.filter(x => x.severity !== 'info');
  res.json({
    checked_sales: sales.length, checked_projects: projects.length,
    issues, errors_count: errors.length, info_count: issues.length - errors.length,
    ok: errors.length === 0,
    formula: 'النهائي = الأساس − الخصومات + التسويات • المدفوع = الدفعات المحصلة + العربون المربوط (مرة واحدة) • المتبقي = النهائي − المدفوع • الربح = الإيراد المحقق − التكلفة − العمولة − المصروفات',
  });
}));

// v1.12: إصلاح الفروق المخزَّنة بإعادة الاحتساب من المحرك المالي (لا يغيّر أي قيد مالي)
app.post('/api/finance/integrity/repair', auth, perm('view_financial_reports', 'add_payment'), wrap((req, res) => {
  const ids = db.prepare("SELECT id FROM sales WHERE status!='cancelled'").all().map(x => x.id);
  let fixed = 0;
  db.transaction(() => {
    for (const id of ids) {
      const before = db.prepare('SELECT final_price,paid_amount,remaining_amount,discount_amount FROM sales WHERE id=?').get(id);
      const f = recalcSale(id);
      recalcCommission(id);
      if (Math.abs(before.paid_amount - f.paid) > 0.01 || Math.abs(before.final_price - f.final) > 0.01 ||
          Math.abs(before.remaining_amount - f.remaining) > 0.01 || Math.abs((before.discount_amount||0) - f.discounts) > 0.01) {
        fixed++;
        financialAudit(req.user, 'integrity_repair', 'sale', id, id, before, { final: f.final, paid: f.paid, remaining: f.remaining, discount: f.discounts }, 'إعادة احتساب من المحرك المالي المركزي');
      }
    }
  })();
  log(req.user, 'integrity_repair', 'system', null, { checked: ids.length, fixed });
  res.json({ ok: true, checked: ids.length, fixed });
}));
...
app.post('/api/sales/:id/discounts', auth, perm('edit_discounts'), wrap((req, res) => {
  const tx = db.transaction(() => {
    ...
    const no = uniqueNo('DSC', 'discounts', 'discount_no');
...
app.post('/api/sales/:id/expenses', auth, perm('edit_sale'), wrap((req, res) => {
  const tx = db.transaction(() => {
    ...
    const no = uniqueNo('EXP', 'expenses', 'expense_no');
