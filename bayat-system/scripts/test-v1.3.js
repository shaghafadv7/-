...
(async () => {
  ...
  ok(schema.d.length === 9 && schema.d.reduce((a, g) => a + g.items.length, 0) === 59, `9 مجموعات و59 صلاحية (${schema.d.length}/${schema.d.reduce((a, g) => a + g.items.length, 0)})`);
  ...
  ok(meV2.d.perms.length === 59, 'تحديد الكل → 59 صلاحية');
