...
(async () => {
  ...
  // v1.12: الفحص صار يفرّق بين الأخطاء والمعلومات (رصيد دائن/دفعات معلقة ليست أخطاء)
  const integErrors = (integ.issues || []).filter(x => x.severity !== 'info');
  ok(integ.ok === true && integErrors.length === 0, `35.1 لا توجد فروق محاسبية (فحص ${integ.checked_sales} صفقة)`, integErrors);
