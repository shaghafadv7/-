...
const statusAr = { available: 'متاح', reserved: 'محجوز', contracted: 'متعاقد', sold: 'مباع', paid: 'مسدد بالكامل', unavailable: 'غير متاح', resale: 'إعادة بيع', owner: 'للمالك', investment: 'استثمار', active: 'نشط', cancelled: 'ملغي', expired: 'منتهي', converted: 'محوّل لبيع', resold: 'أعيد بيعها', under_construction: 'تحت الإنشاء', completed: 'مكتمل', paused: 'متوقف مؤقتًا', draft: 'مسودة', unpaid: 'غير مدفوعة', partial: 'مدفوعة جزئيًا', none: '—' };
...
const docTypeAr = { search_results: 'كشف وحدات', unit_offer: 'عرض وحدة', reservation: 'سند حجز', sale: 'سند بيع', sales_report: 'تقرير المبيعات', payments_report: 'تقرير الدفعات', commissions_report: 'تقرير العمولات', marketers_report: 'تقرير المسوقين', marketer_report: 'تقرير مسوق', financial_report: 'التقرير المالي' };
...
async function boot() {
  try {
    ...
    $$('[data-reservations],[data-sales],[data-payments],[data-marketers],[data-mkreports],[data-customers],[data-reports],[data-docs],[data-users],[data-settings]').forEach(x => x.classList.remove('hidden'));
...
$('#globalSearch').onkeydown = e => { if (e.key === 'Enter') { go('search'); setTimeout(() => { const i = document.querySelector('input[name=search]'); if (i) { i.value = e.target.value; i.form.dispatchEvent(new Event('submit')); } }, 200); } };
...
const pages = {
  ...
  async dashboard() {
    ...
    $('#content').innerHTML = head('لوحة التحكم', 'نظرة مباشرة على المخزون والمبيعات والمالية') +
      ...
      <div class="stat wine"><span class="stat-ic">${icon('percent', 22)}</span><small>عمولات متبقية للمسوقين</small><strong>${money(f.commission_remaining ?? f.commission_due)}</strong><em>${f.marketers} مسوق</em></div></div>` : '') +
  ...
  async sales() {
    ...
    $('#content').innerHTML = head('المبيعات', 'سجل المبيعات مع الخصومات والدفعات') +
      `<section class="panel"><div class="table-wrap"><table><thead><tr><th>رقم البيع</th><th>المشروع / الوحدة</th><th>العميل</th>${showPrice ? '<th>قبل الخصم</th>' : ''}${showDisc && showPrice ? '<th>الخصم</th>' : ''}${showPrice ? '<th>النهائي</th><th>المدفوع</th><th>المتبقي</th>' : ''}<th>المرحلة</th><th>التاريخ</th><th></th></tr></thead><tbody>${rows.map(s => `<tr><td><b>${esc(s.sale_no)}</b></td><td>${esc(s.project_name)} — ${esc(s.unit_number)}</td><td>${esc(s.customer_name)}</td>${showPrice ? `<td>${fmt(s.base_price)}</td>` : ''}${showDisc && showPrice ? `<td>${s.discount_amount ? `<span class="discount-tag">-${fmt(s.discount_amount)}</span>` : '—'}</td>` : ''}${showPrice ? `<td><b>${fmt(s.final_price)}</b></td><td class="money-pos">${fmt(s.paid_amount)}</td><td class="money-neg">${fmt(s.remaining_amount)}</td>` : ''}<td>${s.is_resale ? '<span class="phase-chip">إعادة بيع</span>' : `<span class="phase-chip">${phaseAr[s.sale_phase] || ''}</span>`}</td><td>${esc(s.sale_date)}</td><td class="toolbar">${showPrice && hasPerm('export_pdf') ? `<button class="mini-btn" onclick="saleDoc(${s.id})">سند PDF</button>` : ''}${hasPerm('edit_sale') && s.status === 'active' ? `<button class="mini-btn" onclick="editSaleForm(${s.id})">تعديل</button>` : ''}</td></tr>`).join('') || '<tr><td colspan="11"><div class="empty">لا توجد مبيعات</div></td></tr>'}</tbody></table></div></section>`;
  },
  ...
  async customers() {
    ...
    $('#content').innerHTML = head('العملاء', 'قاعدة العملاء وبيانات التواصل', hasPerm('add_customer') ? '<button class="btn primary" onclick="customerForm()">+ إضافة عميل</button>' : '') +
...
function buildSalePanel(u) {
  ...
  if (showPrice && f && (s.is_resale || s.investor_name)) {
    // ===== المستثمر والأرباح =====
    html += `<h4 class="fin-title">المستثمر والأرباح ${s.investor_name ? '— ' + esc(s.investor_name) : ''}</h4><div class="fin-grid">
      ${finCard('تكلفة الشراء (رأس المال)', money(f.purchase_cost))}
      ${finCard('إجمالي المصروفات', money(f.expenses))}
      ${finCard('العمولات', money(f.commission))}
      ${finCard('إجمالي الإيرادات', money(f.final))}
      ${finCard('الربح الإجمالي', money(f.gross_profit), f.gross_profit >= 0 ? 'profit' : 'loss', (f.gross_profit_pct ?? '—') + '%')}
      ${finCard('صافي الربح', money(f.net_profit), f.net_profit > 0 ? 'profit' : (f.net_profit < 0 ? 'loss' : ''), (f.net_profit_pct ?? '—') + '%')}
    </div>`;
  }
  if (showPrice && !f) {
  ...
  if (showPrice && hasPerm('export_pdf')) finBtns.push(`<button class="btn ghost" onclick="saleDoc(${s.id})">سند بيع PDF</button>`);
...
window.saleForm = async id => {
  ...
  <form id="saleForm" class="form-grid">
    ...
    <div class="field full" style="background:#f7f8fa;border:1px solid var(--line);border-radius:10px;padding:13px 16px">
      ...
      <div class="kv-money" id="calcCommRow" style="display:none"><span>العمولة المستحقة للمسوق</span><b id="calcComm">0</b></div>
    </div>
  ...
  $('#saleForm').onsubmit = async e => {
    ...
    try {
      ...
      closeModal(); toast(`تم تسجيل ${u.status === 'resale' ? 'إعادة البيع' : 'البيع'}: ${d.sale_no} — السعر النهائي ${money(d.final_price)}`);
      go('sales');
    } catch (x) { toast(x.message, true); }
  };
};
window.saleCalc = () => {
  ...
  set('calcBase', base); set('calcDisc', disc); set('calcFinal', final); set('calcRemain', final - paid); set('calcComm', comm);
...
window.investorForm = saleId => {
  ...
  modal(`<h2 class="modal-title">بيانات المستثمر</h2><p class="modal-sub">تُحسب الأرباح ونسبها تلقائيًا من تكلفة الشراء والمصروفات والعمولات</p><form id="invForm" class="form-grid">
    <div class="field"><label>اسم المستثمر<input name="investor_name" value="${esc(s.investor_name || '')}"></label></div>
    <div class="field"><label>تكلفة الشراء (رأس المال)<input type="number" name="purchase_cost" value="${s.purchase_cost ?? ''}" min="0"></label></div>
...
window.marketerDetails = async id => {
  const m = await api('/api/marketers/' + id);
  modal(`<div class="details-hero"><small>${esc(m.code)} • ${m.active ? 'فعال' : 'غير فعال'}</small><h2>${esc(m.name)}</h2><span>${esc(m.phone || 'بدون جوال')}</span></div>
  <div class="stats" style="grid-template-columns:repeat(4,1fr)"><div class="stat"><small>وحدات مباعة</small><strong>${m.units_count}</strong></div><div class="stat green"><small>إجمالي المبيعات</small><strong>${fmt(m.sales_total)}</strong></div><div class="stat orange"><small>عمولات مستحقة</small><strong>${fmt(m.commission_due)}</strong></div><div class="stat wine"><small>عمولات مدفوعة</small><strong>${fmt(m.commission_paid)}</strong></div></div>
  <section class="panel" style="margin-top:16px"><div class="panel-head"><h3>صفقات المسوق</h3></div><div class="table-wrap"><table><thead><tr><th>رقم البيع</th><th>الوحدة</th><th>العميل</th><th>قيمة البيع</th><th>العمولة</th><th>المدفوع</th><th>المتبقي</th><th>الحالة</th></tr></thead><tbody>${m.sales.map(s => `<tr><td><b>${esc(s.sale_no)}</b></td><td>${esc(s.project_name)} — ${esc(s.unit_number)}</td><td>${esc(s.customer_name)}</td><td>${fmt(s.final_price)}</td><td>${fmt(s.commission_total)}</td><td class="money-pos">${fmt(s.commission_paid)}</td><td class="money-neg">${fmt(Math.max(0, s.commission_total - s.commission_paid))}</td><td>${badge(s.commission_status)}</td></tr>`).join('') || '<tr><td colspan="8"><div class="empty">لا توجد صفقات</div></td></tr>'}</tbody></table></div></section>
  <section class="panel"><div class="panel-head"><h3>سجل دفعات المسوق</h3></div><div class="table-wrap"><table><thead><tr><th>الرقم</th><th>التاريخ</th><th>الطريقة</th><th>المرجع</th><th>المبلغ</th><th>الحالة</th></tr></thead><tbody>${m.payments.map(p => `<tr><td>${esc(p.cp_no)}</td><td>${esc(p.pay_date)}</td><td>${methodAr[p.method] || p.method}</td><td>${esc(p.ref_no || '—')}</td><td><b>${fmt(p.amount)}</b></td><td>${p.status === 'active' ? badge('active') : badge('cancelled')}</td></tr>`).join('') || '<tr><td colspan="6"><div class="empty">لا توجد دفعات</div></td></tr>'}</tbody></table></div></section>`, true);
};
