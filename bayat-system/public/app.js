...
const statusAr = { available: 'متاح', reserved: 'محجوز', contracted: 'متعاقد', sold: 'مباع', paid: 'مسدد بالكامل', unavailable: 'غير متاح', resale: 'إعادة بيع', owner: 'للمالك', investment: 'استثمار', active: 'نشط', cancelled: 'ملغي', expired: 'منتهي', converted: 'محوّل لبيع', resold: 'أعيد بيعها', under_construction: 'تحت الإنشاء', completed: 'مكتمل', paused: 'متوقف مؤقتًا', draft: 'مسودة', unpaid: 'غير مدفوعة', partial: 'مدفوعة جزئيًا', open: 'مفتوحة', settled: 'تمت التصفية', overpaid: 'دفع زائد', none: '—' };
...
window.payRefToggle = (methodSel, wrapId, labelId, inputId) => {
  ...
  // v1.12: إظهار الحقول المرتبطة بطريقة الدفع فقط داخل نفس النموذج
  applyMethodFields(sel, m);
};

// v1.12: كل حقل يحمل data-pay-for="cash|bank_transfer|check" يظهر فقط مع طريقته.
// تحويل بنكي: البنك + الحساب المستلم + المرجع • شيك: رقم الشيك + البنك + الاستحقاق + الحالة
// نقدي: المستلم + الخزينة. الحقول المخفية تُفرَّغ حتى لا تُحفظ بيانات غير مرتبطة.
window.applyMethodFields = (scopeEl, method) => {
  const form = scopeEl && (scopeEl.closest('form') || scopeEl.closest('.modal-box'));
  if (!form) return;
  form.querySelectorAll('[data-pay-for]').forEach(el => {
    const allowed = el.getAttribute('data-pay-for').split(/[ ,]+/).filter(Boolean);
    const show = allowed.includes(method);
    el.style.display = show ? '' : 'none';
    el.querySelectorAll('input,select,textarea').forEach(f => {
      if (!show) { f.value = ''; f.required = false; }
      else if (f.dataset.reqFor && f.dataset.reqFor.split(/[ ,]+/).includes(method)) f.required = true;
    });
  });
};
...
const pages = {
  ...
  async dashboard() {
    ...
    $('#content').innerHTML = head('لوحة التحكم', 'نظرة مباشرة على المخزون والمبيعات والمالية') +
      ...
      <div class="stat ${f.net_profit>=0?'green':'wine'}"><small>صافي الربح</small><strong>${money(f.net_profit)}</strong><em>من المبالغ المحصلة فقط</em></div></div>` : '') +
  ...
  async projects() {
    ...
    $('#content').innerHTML = head('المشاريع', 'إنشاء المشاريع وإدارة حالتها ومخزونها', hasPerm('add_project') ? '<button class="btn primary" onclick="projectForm()">+ إضافة مشروع</button>' : '') +
      `<div class="cards">${rows.map(p => `<article class="project-card"><div class="unit-top">${badge(p.construction_status)}<span>${esc(p.code)}</span></div><h3>${esc(p.name)}</h3><p class="loc">${icon('pin', 13)} ${esc(p.location || 'لم يحدد الموقع')}</p><div class="project-kpis"><span><b>${p.floors_count}</b>الأدوار</span><span><b>${p.units_count}</b>الوحدات</span><span><b>${p.available_count}</b>المتاح</span><span><b>${p.resale_count || 0}</b>إعادة بيع</span></div><div class="card-foot"><span>الإنجاز ${p.progress}%</span><span class="toolbar">${hasPerm('view_financial_reports') ? `<button class="mini-btn" onclick="projectAccount(${p.id})">حساب المشروع</button>` : ''}${hasPerm('edit_project') ? `<button class="mini-btn" onclick='projectForm(${JSON.stringify(p)})'>إدارة</button>` : ''}${hasPerm('delete_project') ? `<button class="mini-btn" style="color:#a72f3a" onclick="deleteProject(${p.id},'${esc(p.name)}')">حذف</button>` : ''}</span></div></article>`).join('') || '<div class="empty"><b>لا توجد مشاريع</b>ابدأ بإضافة مشروعك الأول</div>'}</div>`;
  },
...
// v1.10: تحميل الحجوزات مع الفلاتر والجدول المالي الكامل
  ...
  /* ---------- الدفعات ---------- */
  async payments(tab = 'ledger') {
    // v1.12: شاشة دفعات واحدة موحدة (السجل المالي الشامل: حجز + بيع)
    // الجدول يعرض الأساسيات فقط، والتفاصيل تظهر عند الضغط على الدفعة.
    const projects = hasPerm('view_projects') ? await api('/api/projects').catch(() => []) : [];
    $('#content').innerHTML = head('الدفعات', 'كل الدفعات (عربون وبيع) بمصدر واحد — اضغط أي دفعة لعرض تفاصيلها الكاملة') +
      `<section class="panel"><div class="panel-body"><div class="filters-unified">
        <div class="field"><label>المشروع<select id="ldProject"><option value="">كل المشاريع</option>${projects.map(p => `<option value="${p.id}">${esc(p.name)}</option>`).join('')}</select></label></div>
        <div class="field"><label>العميل<input id="ldCustomer" placeholder="اسم العميل"></label></div>
        <div class="field"><label>العقار / الوحدة<input id="ldUnit" placeholder="رقم الوحدة"></label></div>
        <div class="field"><label>طريقة الدفع<select id="ldMethod"><option value="">الكل</option><option value="cash">نقدي</option><option value="bank_transfer">تحويل بنكي</option><option value="check">شيك</option></select></label></div>
        <div class="field"><label>البنك<input id="ldBank" placeholder="اسم البنك"></label></div>
        <div class="field"><label>الحساب المستلم<input id="ldAccount2" placeholder="الحساب / الخزينة"></label></div>
        <div class="field"><label>المستلم<input id="ldRecipient" placeholder="اسم المستلم"></label></div>
        <div class="field"><label>حالة الدفعة<select id="ldStatus"><option value="">الكل</option><option value="received">مستلمة</option><option value="deposited">مودعة</option><option value="pending">معلقة</option><option value="refunded">مرتجعة</option><option value="cancelled">ملغاة</option></select></label></div>
        ...
        <div class="field"><label>من مبلغ<input type="number" id="ldAmtFrom" placeholder="0"></label></div>
        <div class="field"><label>إلى مبلغ<input type="number" id="ldAmtTo" placeholder="0"></label></div>
        <div class="field"><label>رقم المرجع<input id="ldRef" placeholder="رقم الحوالة / الشيك"></label></div>
        <div class="field"><label>رقم العملية<input id="ldOp" placeholder="رقم الحجز / البيع"></label></div>
        <div class="field"><label>النوع<select id="ldType"><option value="">الكل</option><option value="reservation">عربون حجز</option><option value="sale">دفعة بيع</option></select></label></div>
        <div class="filter-actions">
          <button class="btn primary" onclick="loadPaymentLedger()">تطبيق الفلاتر</button>
          <button class="btn ghost" onclick="resetPaymentFilters()">إعادة تعيين الفلاتر</button>
          ${hasPerm('export_pdf') ? '<button class="btn ghost" onclick="paymentsReportPdf(false)">تصدير PDF</button>' : ''}
          ${hasPerm('save_pdf') ? '<button class="btn ghost" onclick="paymentsReportPdf(true)">حفظ في الأرشيف</button>' : ''}
        </div>
  ...
  async marketers(tab = 'list') {
    ...
    if (window._mkTab !== 'commissions' && canList) {
      ...
      $('#content').innerHTML = head('العمولات', 'متابعة عمولات المسوقين ودفعاتها') + `<div class="tabs">${tabs}</div>` +
        `<section class="panel"><div class="table-wrap"><table><thead><tr><th>المسوق</th><th>رقم البيع</th><th>المشروع / الوحدة</th><th>قيمة البيع</th><th>العمولة المستحقة</th><th>المدفوع للمسوق</th><th>المتبقي للمسوق</th><th>الحالة</th><th></th></tr></thead><tbody>${cms.map(c => `<tr><td><b>${esc(c.marketer_name || '')}</b> <small>${esc(c.marketer_code || '')}</small></td><td>${esc(c.sale_no)}</td><td>${esc(c.project_name)} — ${esc(c.unit_number)}</td><td>${fmt(c.final_price)}</td><td><b>${fmt(c.commission_total)}</b></td><td class="money-pos">${fmt(c.commission_paid)}</td><td class="money-neg">${fmt(c.commission_due)}</td><td>${badge(c.commission_status)}</td><td class="toolbar">${hasPerm('edit_marketer') && hasPerm('view_commissions') ? `<button class="mini-btn" onclick="commissionForm(${c.id})">تعديل</button>` : ''}${hasPerm('pay_commission') && c.commission_due > 0 ? `<button class="mini-btn" onclick="commissionPayForm(${c.id},${c.commission_due})">دفع</button>` : ''}</td></tr>`).join('') || '<tr><td colspan="9"><div class="empty">لا توجد عمولات مسجلة</div></td></tr>'}</tbody></table></div></section>`;
    }
  },
  ...
  async customerFinance() {
    ...
    // v1.12: فلاتر حساب العميل الكاملة — العميل/المشروع/العقار/حالة الصفقة/الفترة/حالة السداد
    $('#content').innerHTML = head('العملاء والمتبقيات المالية','ملخص موحد لكل عميل: قيمة العقود ← المدفوع ← المتبقي — بمصدر حساب واحد') +
      `<section class="panel"><div class="panel-body"><div class="filters-unified">
      <div class="field"><label>العميل / بحث<input id="cfSearch" placeholder="اسم العميل / جوال / رقم بيع"></label></div>
      ...
      <div class="field"><label>العقار / الوحدة<input id="cfUnit" placeholder="رقم الوحدة"></label></div>
      <div class="field"><label>حالة الصفقة<select id="cfSaleStatus"><option value="">الكل</option><option value="active">نشطة</option><option value="resold">أعيد بيعها</option></select></label></div>
      <div class="field"><label>حالة السداد<select id="cfSettle"><option value="">الكل</option><option value="unpaid">لم يسدد</option><option value="partial">سداد جزئي</option><option value="settled">مسدد بالكامل</option><option value="overpaid">دفع زائد</option></select></label></div>
      <div class="field"><label>من تاريخ<input type="date" id="cfFrom"></label></div>
      <div class="field"><label>إلى تاريخ<input type="date" id="cfTo"></label></div>
      <div class="field"><label class="check" style="margin-top:22px"><input type="checkbox" id="cfOutstanding"> عليهم مبالغ فقط</label></div>
      <div class="filter-actions"><button class="btn primary" onclick="loadCustomerFinance()">تطبيق الفلاتر</button><button class="btn ghost" onclick="resetCustomerFilters()">إعادة تعيين الفلاتر</button></div>
  ...
  // v1.12: صفحة تصفيات واحدة بتبويبين (دمج تصفيات المستثمرين وتصفية المسوقين)
  async investorSettlements(tab = 'investors') {
    const active = (typeof tab === 'string' && tab) ? tab : 'investors';
    const tabs = `<div class="tabs"><button class="${active === 'investors' ? 'active' : ''}" onclick="go('investorSettlements','investors')">تصفيات المستثمرين</button><button class="${active === 'marketers' ? 'active' : ''}" onclick="go('investorSettlements','marketers')">تصفية المسوقين</button></div>`;
    if (active === 'marketers') {
      cache.marketers = await api('/api/marketers').catch(() => []);
      $('#content').innerHTML = head('التصفيات', 'تجميع عمولات المسوق غير المصفاة في تصفية واحدة دون ازدواجية') + tabs +
        `<section class="panel"><div class="panel-body"><div class="filters-unified"><div class="field"><label>المسوق<select id="msMarketer"><option value="">اختر المسوق</option>${cache.marketers.map(m => `<option value="${m.id}">${esc(m.name)} — ${esc(m.code)}</option>`).join('')}</select></label></div><div class="filter-actions"><button class="btn primary" onclick="loadMarketerSettlement()">عرض العمولات غير المصفاة</button></div></div></div></section><div id="settlementBody"><div class="empty">اختر المسوق لعرض كشف التصفية</div></div>`;
      return;
    }
    $('#content').innerHTML = head('التصفيات', 'عمليات إعادة البيع لصالح العميل أو المستثمر، مرتبطة بفواتير البيع والتصفية الأصلية') + tabs + '<div id="investorSettlementBody"><div class="empty">جارٍ تحميل العمليات...</div></div>';
    const rows = await api('/api/investor-settlements'), b = $('#investorSettlementBody');
    b.innerHTML = `<div class="filter-summary"><span class="fs-item"><small>عمليات إعادة البيع</small><b>${rows.length}</b></span><span class="fs-sep"></span><span class="fs-item"><small>صافي مستحق المستثمرين</small><b>${money(rows.reduce((a, r) => a + r.net_amount, 0))}</b></span><span class="fs-sep"></span><span class="fs-item"><small>تمت تصفيته</small><b class="money-pos">${money(rows.reduce((a, r) => a + r.settled_amount, 0))}</b></span><span class="fs-sep"></span><span class="fs-item"><small>المتبقي</small><b class="money-neg">${money(rows.reduce((a, r) => a + r.investor_remaining, 0))}</b></span></div><section class="panel"><div class="table-wrap"><table><thead><tr><th>فاتورة التصفية</th><th>فاتورة / رقم البيع</th><th>المشروع / الوحدة</th><th>المالك السابق</th><th>المالك الجديد</th><th>قيمة البيع</th><th>صافي مستحق المستثمر</th><th>المصفى</th><th>المتبقي للمستثمر</th><th>الحالة</th><th></th></tr></thead><tbody>${rows.map(r => `<tr><td><b>${esc(r.invoice_no || r.report_no)}</b><br><small>${esc(r.report_no)}</small></td><td>${esc(r.sale_invoice_no || '—')}<br><small>${esc(r.sale_no)}</small></td><td>${esc(r.project_name)} — ${esc(r.unit_number)}</td><td>${esc(r.prev_owner_name || r.beneficiary_name || '—')}</td><td>${esc(r.new_owner_name)}</td><td>${money(r.final_price)}</td><td><b>${money(r.net_amount)}</b></td><td class="money-pos">${money(r.settled_amount)}</td><td class="money-neg">${money(r.investor_remaining)}</td><td>${badge(r.investor_status)}</td><td class="toolbar"><button class="mini-btn" onclick="showSettlementReport(${r.id})">عرض التصفية</button>${hasPerm('manage_settlements') && r.investor_remaining > 0 ? `<button class="mini-btn" onclick="settlementPayForm(${r.id},${r.entry_id},'${esc(r.prev_owner_name || 'المستثمر')}',${r.investor_remaining})">تسجيل التصفية</button>` : ''}${hasPerm('export_pdf') ? `<button class="mini-btn" onclick="settlementPdf(${r.id},false)">تنزيل PDF</button>` : ''}</td></tr>`).join('') || '<tr><td colspan="11"><div class="empty">لا توجد عمليات إعادة بيع لصالح مستثمر</div></td></tr>'}</tbody></table></div></section>`;
    decorateIcons(b);
  },

  // توافق خلفي: أي رابط قديم لتصفية المسوقين يفتح التبويب المدمج
  async marketerSettlements() { return pages.investorSettlements('marketers'); },
  ...
  async settings() {
    ...
    $('#content').innerHTML = head('إعدادات النظام', 'بيانات الشركة المركزية: الهوية والتواصل والنسخ الاحتياطي') +
      ...
      <form id="brandForm" class="panel-body"><div class="form-grid">
        ...
        <div class="field full"><label>فرض الحقول المحاسبية الكاملة لكل طريقة دفع<select name="strict_payment_fields"><option value="0" ${settings.strict_payment_fields !== '1' ? 'selected' : ''}>مرن — يسمح بحفظ الدفعة مع نقص بعض البيانات (الافتراضي)</option><option value="1" ${settings.strict_payment_fields === '1' ? 'selected' : ''}>صارم — يمنع حفظ دفعة ناقصة (تحويل: البنك والحساب • شيك: البنك والاستحقاق • نقدي: المستلم)</option></select></label></div>
...
const LEDGER_FILTER_MAP = {
  ldQ: 'q', ldType: 'type', ldFrom: 'date_from', ldTo: 'date_to', ldMethod: 'method',
  ldStatus: 'payment_status', ldAccount: 'account', ldProject: 'project_id', ldCustomer: 'customer',
  ldUnit: 'unit_number', ldBank: 'bank', ldAccount2: 'deposit_account', ldRecipient: 'recipient',
  ldAmtFrom: 'amount_from', ldAmtTo: 'amount_to', ldRef: 'ref_no', ldOp: 'operation_no',
};
window.resetPaymentFilters = () => {
  Object.keys(LEDGER_FILTER_MAP).forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  loadPaymentLedger();
};
// v1.12: جدول الدفعات المبسّط — الأساسيات فقط، والتفاصيل عند الضغط على الصف
window.loadPaymentLedger = async () => {
  const body = document.getElementById('ledgerBody');
  if (!body) return;
  const q = new URLSearchParams();
  for (const [el, key] of Object.entries(LEDGER_FILTER_MAP)) {
    const v = document.getElementById(el)?.value?.trim() || '';
    if (v) q.set(key, v);
  }
  const d = await api('/api/finance/payment-ledger?' + q.toString());
  const t = d.totals;
  body.innerHTML = `<div class="filter-summary">
      <span class="fs-item"><small>عدد النتائج</small><b>${t.count}</b></span><span class="fs-sep"></span>
      <span class="fs-item"><small>إجمالي المبالغ</small><b>${money(t.amount)}</b></span><span class="fs-sep"></span>
      <span class="fs-item"><small>المحصّل فعليًا</small><b class="money-pos">${money(t.collected)}</b></span>
      ${t.pending ? `<span class="fs-sep"></span><span class="fs-item"><small>معلّقة</small><b>${money(t.pending)}</b></span>` : ''}
      ${t.refunded ? `<span class="fs-sep"></span><span class="fs-item"><small>مرتجعة</small><b>${money(t.refunded)}</b></span>` : ''}
      ${t.cancelled ? `<span class="fs-sep"></span><span class="fs-item"><small>ملغاة</small><b>${money(t.cancelled)}</b></span>` : ''}
    </div>
    <section class="panel"><div class="table-wrap"><table><thead><tr>
      <th>التاريخ</th><th>رقم العملية</th><th>العميل</th><th>المشروع</th><th>العقار</th>
      <th>المبلغ</th><th>طريقة الدفع</th><th>المرجع</th><th>الحساب المستلم</th><th>الحالة</th>
    </tr></thead><tbody>${d.rows.map(x => `<tr class="row-click" onclick="paymentTrailModal('${x.type}',${x.source_id})" title="اضغط لعرض تفاصيل الدفعة">
      <td>${esc(x.pay_date)}</td>
      <td><b>${esc(x.payment_no)}</b><br><small>${esc(x.operation_no)}</small></td>
      <td>${esc(x.customer_name)}</td>
      <td>${esc(x.project_name)}</td>
      <td>${esc(x.unit_number)}</td>
      <td><b>${money(x.amount)}</b></td>
      <td>${methodAr[x.method] || x.method}</td>
      <td>${esc(x.ref_no || '—')}</td>
      <td>${esc(x.deposit_account || x.bank || '—')}</td>
      <td>${x.status === 'cancelled' ? '<span class="badge cancelled">ملغاة</span>' : `<span class="phase-chip">${paymentStatusAr[x.payment_status] || x.payment_status}</span>`}</td>
    </tr>`).join('') || '<tr><td colspan="10"><div class="empty">لا توجد دفعات ضمن الفلاتر</div></td></tr>'}</tbody></table></div></section>`;
  decorateIcons(body);
};
...
/* ============================================================
   ...
   ============================================================ */
function invoiceSaleHtml(d){const s=d.sale,f=d.finance,b=d.beneficiary;return `<div class="invoice-head"><div><small>فاتورة بيع عقارية</small><h2>${esc(s.invoice_no||'—')}</h2><span>رقم البيع: ${esc(s.sale_no)} • ${esc(s.sale_date)}</span></div><div>${badge(s.status)}</div></div><div class="details-grid"><div class="detail"><small>المشروع / الوحدة</small><b>${esc(s.project_name)} — ${esc(s.unit_number)}</b></div><div class="detail"><small>العميل / المالك</small><b>${esc(s.customer_name)}</b></div><div class="detail"><small>نوع العملية</small><b>${s.is_resale?'إعادة بيع / بيع ثانٍ':'بيع أول'}</b></div><div class="detail"><small>الجهة المستفيدة</small><b>${esc(b.name)}</b></div><div class="detail"><small>الحساب المستفيد</small><b>${esc(b.account)}</b></div><div class="detail"><small>السعر النهائي</small><b>${money(f.final)}</b></div></div><div class="profit-kpis"><div class="fin-card strong"><small>السعر الأساسي</small><b>${money(f.base)}</b></div><div class="fin-card disc"><small>الخصومات</small><b>${money(f.discounts)}</b></div><div class="fin-card pos"><small>إجمالي المدفوع</small><b>${money(f.paid)}</b></div><div class="fin-card due"><small>المتبقي على العميل</small><b>${money(Math.max(0,f.remaining))}</b></div><div class="fin-card"><small>العمولة</small><b>${money(f.commission)}</b></div><div class="fin-card"><small>صافي الربح</small><b>${money(f.net_profit)}</b></div></div>
...
const CF_FILTERS={cfSearch:'search',cfProject:'project_id',cfUnit:'unit_number',cfSaleStatus:'sale_status',cfSettle:'settlement_status',cfFrom:'date_from',cfTo:'date_to'};
window.resetCustomerFilters=()=>{Object.keys(CF_FILTERS).forEach(id=>{const e=document.getElementById(id);if(e)e.value='';});const o=document.getElementById('cfOutstanding');if(o)o.checked=false;loadCustomerFinance();};
window.loadCustomerFinance=async()=>{
  const q=new URLSearchParams();
  for(const [el,key] of Object.entries(CF_FILTERS)){const v=document.getElementById(el)?.value?.trim()||'';if(v)q.set(key,v);}
  if($('#cfOutstanding')?.checked)q.set('outstanding','1');
  const d=await api('/api/customer-finance?'+q),b=$('#customerFinanceBody');if(!b)return;
  b.innerHTML=`<div class="filter-summary">
      <span class="fs-item"><small>عدد النتائج</small><b>${d.totals.count}</b></span><span class="fs-sep"></span>
      <span class="fs-item"><small>إجمالي قيمة العقود</small><b>${money(d.totals.sales)}</b></span><span class="fs-sep"></span>
      <span class="fs-item"><small>إجمالي المدفوع</small><b class="money-pos">${money(d.totals.paid)}</b></span><span class="fs-sep"></span>
      <span class="fs-item"><small>إجمالي المتبقي</small><b class="money-neg">${money(d.totals.remaining)}</b></span>
      ${d.totals.credit?`<span class="fs-sep"></span><span class="fs-item"><small>أرصدة زائدة للعملاء</small><b>${money(d.totals.credit)}</b></span>`:''}
    </div>
  <section class="panel"><div class="table-wrap"><table><thead><tr><th>العميل</th><th>المشروع / الوحدة</th><th>نوع البيع</th><th>قيمة العقد</th><th>المدفوع</th><th>المتبقي</th><th>آخر دفعة</th><th>حالة السداد</th><th></th></tr></thead><tbody>${d.rows.map(r=>`<tr><td><b>${esc(r.customer_name)}</b><br><small>${esc(r.phone)}</small></td><td>${esc(r.project_name)} — ${esc(r.unit_number)}<br><small>${esc(r.invoice_no||'—')} • ${esc(r.sale_no)}</small></td><td>${r.is_resale?'إعادة بيع':'بيع أول'}</td><td>${money(r.final_price)}</td><td class="money-pos">${money(r.paid_amount)}</td><td class="money-neg">${money(Math.max(0,r.remaining_amount))}${r.client_credit>0?`<br><small>رصيد له: ${money(r.client_credit)}</small>`:''}</td><td>${r.last_payment_date?`${esc(r.last_payment_date)} — ${money(r.last_payment_amount)}`:'—'}</td><td>${badge(r.settlement_status)}</td><td class="toolbar"><button class="mini-btn" onclick="customerStatement(${r.customer_id})">كشف الحساب</button><button class="mini-btn" onclick="viewInvoice(${r.sale_id})">الفاتورة</button>${hasPerm('export_pdf')?`<button class="mini-btn" onclick="invoicePdf(${r.sale_id})">PDF</button>`:''}</td></tr>`).join('')||'<tr><td colspan="9"><div class="empty">لا توجد نتائج مطابقة للفلاتر</div></td></tr>'}</tbody></table></div></section>`;decorateIcons(b);
};
// v1.12: كشف حساب العميل — ملخص هرمي واضح ثم جدول العقارات (بلا تكرار للأرقام)
window.customerStatement=async id=>{
  const d=await api('/api/customer-finance/'+id);
  const t=d.totals;
  modal(`<div class="details-hero"><small>كشف حساب العميل</small><h2>${esc(d.customer.name)}</h2><span>${esc(d.customer.phone)}${d.customer.email?' • '+esc(d.customer.email):''}</span></div>
<div class="profit-kpis">
  <div class="fin-card"><small>عدد العقارات</small><b>${t.units}</b></div>
  <div class="fin-card strong"><small>إجمالي قيمة العقود</small><b>${money(t.sales)}</b></div>
  <div class="fin-card pos"><small>إجمالي المدفوع</small><b>${money(t.paid)}</b></div>
  <div class="fin-card due"><small>إجمالي المتبقي</small><b>${money(t.remaining)}</b></div>
  ${t.credit>0?`<div class="fin-card"><small>رصيد زائد للعميل</small><b>${money(t.credit)}</b></div>`:''}
</div>
<section class="panel"><div class="panel-head"><h3>العقارات</h3><span class="perm-count">${d.sales.length} عقار</span></div><div class="table-wrap"><table><thead><tr><th>البيع</th><th>المشروع / الوحدة</th><th>قيمة العقد</th><th>الخصم</th><th>السعر النهائي</th><th>المدفوع</th><th>المتبقي</th><th>العمولة</th><th>المصروفات</th><th>الحالة</th></tr></thead><tbody>${d.sales.map(x=>{const f=x.finance;return `<tr class="row-click" onclick="closeModal();viewInvoice(${x.id})" title="اضغط لعرض تفاصيل الصفقة والدفعات"><td><b>${esc(x.sale_no)}</b><br><small>${x.is_resale?'إعادة بيع':'بيع أول'}</small></td><td>${esc(x.project_name)} — ${esc(x.unit_number)}</td><td>${money(f.base)}</td><td>${f.discounts?money(f.discounts):'—'}</td><td><b>${money(f.final)}</b></td><td class="money-pos">${money(f.paid)}</td><td class="money-neg">${money(Math.max(0,f.remaining))}</td><td>${f.commission?money(f.commission):'—'}</td><td>${f.expenses?money(f.expenses):'—'}</td><td>${badge(f.client_state==='settled'?'settled':(f.client_state==='due_to_client'?'overpaid':(f.paid>0?'partial':'unpaid')))}</td></tr>`;}).join('')||'<tr><td colspan="10"><div class="empty">لا توجد عقارات</div></td></tr>'}</tbody></table></div></section>
<section class="panel"><div class="panel-head"><h3>سجل الحركات</h3><span class="perm-count">${d.movements.length} حركة</span></div><div class="table-wrap"><table><thead><tr><th>التاريخ</th><th>المرجع</th><th>العملية</th><th>البيع / الوحدة</th><th>المبلغ</th><th>الحالة</th></tr></thead><tbody>${d.movements.map(m=>`<tr><td>${esc(m.movement_date||'')}</td><td>${esc(m.reference)}</td><td>${esc(m.title)}</td><td>${esc(m.sale_no)} — ${esc(m.unit_number)}</td><td>${m.amount?money(m.amount):'—'}</td><td>${m.status==='active'?badge('active'):badge('cancelled')}</td></tr>`).join('')||'<tr><td colspan="6"><div class="empty">لا توجد حركات</div></td></tr>'}</tbody></table></div></section>`,true);
};
// v1.12: حساب المشروع — ملخص إداري سريع ثم جدول العمليات (بلا تفاصيل داخلية في الصدارة)
window.projectAccount = async (projectId, from = '', to = '') => {
  const q = new URLSearchParams({ project_id: projectId });
  if (from) q.set('date_from', from); if (to) q.set('date_to', to);
  const d = await api('/api/reports/project-account?' + q);
  const s = d.summary;
  modal(`<div class="details-hero"><small>حساب المشروع</small><h2>${esc(d.project ? d.project.name : 'كل المشاريع')}</h2><span>${d.filters.map(f => f[0] + ': ' + f[1]).join(' • ') || 'كل الفترات'}</span></div>
  <div class="profit-kpis">
    <div class="fin-card"><small>عدد الوحدات</small><b>${s.units_total}</b></div>
    <div class="fin-card"><small>المباع</small><b>${s.units_sold}</b></div>
    <div class="fin-card"><small>المحجوز</small><b>${s.units_reserved}</b></div>
    <div class="fin-card"><small>المتاح</small><b>${s.units_available}</b></div>
    <div class="fin-card strong"><small>قيمة المبيعات</small><b>${money(s.sales_value)}</b></div>
    <div class="fin-card pos"><small>التحصيل</small><b>${money(s.collected)}</b></div>
    <div class="fin-card due"><small>المتبقي</small><b>${money(s.remaining)}</b></div>
    <div class="fin-card disc"><small>المصروفات</small><b>${money(s.expenses)}</b></div>
    <div class="fin-card"><small>العمولات</small><b>${money(s.commissions)}</b></div>
    <div class="fin-card ${s.net_profit >= 0 ? 'profit' : 'loss'}"><small>صافي الربح</small><b>${money(s.net_profit)}</b></div>
  </div>
  <section class="panel"><div class="panel-head"><h3>العمليات</h3><span class="perm-count">${d.rows.length} عملية</span></div><div class="table-wrap"><table><thead><tr><th>التاريخ</th><th>البيع</th><th>العميل</th><th>الوحدة</th><th>السعر النهائي</th><th>المدفوع</th><th>المتبقي</th><th>العمولة</th><th>المصروفات</th><th>صافي الربح</th></tr></thead><tbody>${d.rows.map(r => `<tr class="row-click" onclick="closeModal();viewInvoice(${r.sale_id})"><td>${esc(r.sale_date)}</td><td><b>${esc(r.sale_no)}</b></td><td>${esc(r.customer_name)}</td><td>${esc(r.unit_number)}</td><td>${money(r.final)}</td><td class="money-pos">${money(r.paid)}</td><td class="money-neg">${money(Math.max(0, r.remaining))}</td><td>${r.commission ? money(r.commission) : '—'}</td><td>${r.expenses ? money(r.expenses) : '—'}</td><td class="${r.net_profit >= 0 ? 'money-pos' : 'money-neg'}"><b>${money(r.net_profit)}</b></td></tr>`).join('') || '<tr><td colspan="10"><div class="empty">لا توجد عمليات</div></td></tr>'}</tbody></table></div></section>`, true);
};
window.loadProfitDashboard=async()=>{const q=new URLSearchParams(),f=$('#pfFrom')?.value,t=$('#pfTo')?.value,p=$('#pfProject')?.value;if(f)q.set('date_from',f);if(t)q.set('date_to',t);if(p)q.set('project_id',p);const [d,audit]=await Promise.all([api('/api/finance/profit-dashboard?'+q),api('/api/finance/audit?'+q)]),x=d.totals,b=$('#profitBody');if(!b)return;b.innerHTML=`<div class="profit-formula">${esc(d.formula)}</div><div class="profit-kpis"><div class="fin-card strong"><small>إجمالي المبيعات</small><b>${money(x.sales)}</b></div><div class="fin-card pos"><small>المبالغ المحصلة</small><b>${money(x.collected)}</b></div><div class="fin-card due"><small>المتبقي على العملاء</small><b>${money(x.receivables)}</b></div><div class="fin-card"><small>تكلفة العقارات</small><b>${money(x.property_cost)}</b></div><div class="fin-card disc"><small>المصروفات الأخرى</small><b>${money(x.expenses)}</b></div><div class="fin-card"><small>عمولات المسوقين</small><b>${money(x.commissions)}</b></div><div class="fin-card loss"><small>إجمالي التكاليف</small><b>${money(x.total_costs)}</b></div><div class="fin-card ${x.net_profit>=0?'profit':'loss'}"><small>صافي الربح</small><b>${money(x.net_profit)}</b></div></div>
...
// v1.12.2: تفصيل «من أين جاء صافي مستحق المالك السابق» — كل بند خصم على حدة
function sellerBreakdownHtml(d){
  const b=d.seller_breakdown; if(!b) return '';
  return `<section class="panel"><div class="panel-head"><h3>احتساب صافي مستحق المالك السابق (المستثمر)</h3><span class="perm-count">${esc(b.formula)}</span></div>
  <div class="table-wrap"><table><thead><tr><th>البيان</th><th>التفصيل</th><th>المبلغ</th></tr></thead><tbody>
    <tr><td><b>السعر النهائي للبيع الثاني</b></td><td>—</td><td><b>${money(b.final_price)}</b></td></tr>
    ${b.deductions.map(x=>`<tr><td>ناقص: ${esc(x.label)}</td><td>${x.count>1?x.count+' بنود':'—'}</td><td class="money-neg">− ${money(x.amount)}</td></tr>`).join('')||'<tr><td colspan="3">لا توجد خصومات</td></tr>'}
    <tr style="background:#f7f8fa"><td><b>إجمالي الخصومات</b></td><td>—</td><td class="money-neg"><b>${money(b.total_deductions)}</b></td></tr>
    <tr style="background:#eef6f1"><td><b>صافي المستحق للمالك السابق</b></td><td>—</td><td class="money-pos"><b>${money(b.net_to_seller)}</b></td></tr>
  </tbody></table></div></section>`;
}
window.showSettlementReport = async id => {
  ...
  ${(d.expenses&&d.expenses.length)?`<section class="panel"><div class="panel-head"><h3>تفصيل مصروفات الصفقة</h3><span class="perm-count">${d.expenses.length} بند</span></div><div class="table-wrap"><table><thead><tr><th>رقم المصروف</th><th>البيان</th><th>التاريخ</th><th>المبلغ</th><th>ملاحظات</th></tr></thead><tbody>${d.expenses.map(x=>`<tr><td><b>${esc(x.expense_no)}</b></td><td>${esc(x.title)}</td><td>${esc(x.expense_date||'—')}</td><td>${money(x.amount)}</td><td>${esc(x.notes||'—')}</td></tr>`).join('')}<tr style="background:#f7f8fa;font-weight:800"><td colspan="3">الإجمالي</td><td>${money(d.expenses.reduce((a,x)=>a+x.amount,0))}</td><td></td></tr></tbody></table></div></section>`:''}
  ${(d.commission_payments&&d.commission_payments.length)?`<section class="panel"><div class="panel-head"><h3>دفعات عمولة المسوق</h3><span class="perm-count">${d.commission_payments.length} دفعة</span></div><div class="table-wrap"><table><thead><tr><th>رقم الدفعة</th><th>المسوق</th><th>التاريخ</th><th>المبلغ</th><th>الحالة</th></tr></thead><tbody>${d.commission_payments.map(x=>`<tr><td><b>${esc(x.cp_no)}</b></td><td>${esc(x.marketer_name||'—')}</td><td>${esc(x.pay_date)}</td><td>${money(x.amount)}</td><td>${badge(x.status==='active'?'settled':'cancelled')}</td></tr>`).join('')}</tbody></table></div></section>`:''}
...
function buildSalePanel(u) {
  ...
  if (showPrice && f) {
    ...
    html += `<h4 class="fin-title">اللوحة المالية</h4><div class="fin-grid">
      ...
      ${finCard('السعر النهائي', money(f.final), 'strong')}
  ...
  if (showPrice && f) {
    ...
    html += `<h4 class="fin-title">الإيرادات والتكاليف والأرباح ${s.investor_name ? '— ' + esc(s.investor_name) : ''}</h4><div class="fin-grid">
      ${finCard('الإيراد المحقق', money(f.realized_revenue), 'pos', 'لا يشمل المتبقي')}
      ...
      ${finCard('صافي الربح', money(f.net_profit), f.net_profit >= 0 ? 'profit' : 'loss', 'على أساس التحصيل الفعلي')}
    </div>`;
  }
  if (showPrice && !f) {
    html += `<div style="margin-top:14px">
      <div class="kv-money"><span>السعر الأساسي (قبل الخصم)</span><b>${money(s.base_price ?? s.list_price)}</b></div>
      <div class="kv-money"><span>السعر النهائي</span><b>${money(s.final_price)}</b></div>
      <div class="kv-money"><span>المبلغ المدفوع</span><b class="money-pos">${money(s.paid_amount)}</b></div>
      <div class="kv-money total"><span>المتبقي على العميل</span><b class="money-neg">${money(s.remaining_amount)}</b></div></div>`;
  }
...
window.saleForm = async (id, reservationId) => {
  ...
  const marketerOpts = marketers.map(m => `<option value="${m.id}">${esc(m.name)} — ${esc(m.code)}</option>`).join('');
  const depBox = depositTotal > 0 ? `<div class="deposit-box"><h4>العربون المدفوع سابقًا — يُحتسب ضمن إجمالي المدفوع ولا يُسجل مرة أخرى</h4>
  ...
  <form id="saleForm" class="form-grid">
    ...
    <div class="field full" style="background:#f7f8fa;border:1px solid var(--line);border-radius:10px;padding:13px 16px">
      ...
      <div class="kv-money total"><span>السعر النهائي</span><b id="calcFinal">0</b></div>
      ${depositTotal > 0 ? `<div class="kv-money"><span>العربون المدفوع سابقًا (${depositPayments.length} قيد)</span><b id="calcDeposit" style="color:#315f91">${money(depositTotal)}</b></div>` : ''}
      <div class="kv-money"><span>${depositTotal > 0 ? 'إجمالي المدفوع (العربون + الدفعة الإضافية)' : 'إجمالي المدفوع'}</span><b id="calcPaid" style="color:#1e6b4f">0</b></div>
      <div class="kv-money"><span>المتبقي على العميل</span><b id="calcRemain">0</b></div>
      <div class="kv-money" id="calcCommRow" style="display:none"><span>عمولة المسوق</span><b id="calcComm">0</b></div>
      <div class="kv-money"><span>صافي الربح</span><b id="calcProfit">0</b></div>
    </div>
...
window.saleCalc = () => {
  ...
  set('calcBase', base); set('calcDisc', disc); set('calcFinal', final);
  set('calcPaid', totalPaid);      // إجمالي المدفوع = العربون + الدفعة الإضافية (بحد أقصى السعر النهائي)
  set('calcRemain', remaining);    // المتبقي على العميل = السعر النهائي − إجمالي المدفوع
  set('calcComm', comm); set('calcProfit', totalPaid - cost - comm);
...
window.editSaleForm = async saleId => {
  ...
  modal(`<h2 class="modal-title">تعديل الصفقة ${esc(s.sale_no)}</h2><form id="editSaleForm" class="form-grid">
    ...
    <div class="field"><label>السعر النهائي<input id="esFinal" value="${s.final_price}" disabled style="background:#f4f5f7"></label></div>` : ''}
...
window.paymentForm = saleId => {
  modal(`<h2 class="modal-title">تسجيل دفعة جديدة</h2><p class="modal-sub">سيُحدَّث المدفوع والمتبقي وحالة السداد تلقائيًا في كل الشاشات والتقارير</p><form id="paymentForm" class="form-grid">
    ...
    <div class="field" data-pay-for="bank_transfer check" style="display:none"><label>البنك<input name="bank" placeholder="مثال: مصرف الراجحي"></label></div>
    <div class="field" data-pay-for="check" style="display:none"><label>تاريخ استحقاق الشيك<input type="date" name="check_due_date"></label></div>
    <div class="field" data-pay-for="check" style="display:none"><label>حالة الشيك<select name="check_status"><option value="pending">قيد التحصيل</option><option value="cleared">محصّل</option><option value="bounced">مرتجع</option></select></label></div>
    <div class="field"><label>حالة الدفعة<select name="payment_status"><option value="received">مستلمة</option><option value="deposited">مودعة</option><option value="pending">معلقة</option></select></label></div>
    <div class="field full"><label>ملاحظات<textarea name="notes"></textarea></label></div>
    ${payReceiverBox()}
  ...
  applyMethodFields(document.getElementById('pfMethod'), 'cash');
...
// v1.12: تقرير الدفعات يُصدَّر بنفس فلاتر الشاشة تمامًا (الشاشة = PDF = Excel)
window.currentPaymentFilters = () => {
  const out = {};
  for (const [el, key] of Object.entries(LEDGER_FILTER_MAP)) {
    const v = document.getElementById(el)?.value?.trim() || '';
    if (v) out[key] = v;
...
window.paymentsReportPdf = save => exportDoc('payments_report', currentPaymentFilters(), save);
...
window.printUnitOffer = async id => {
  ...
  printHTML('عرض وحدة سكنية', `${u.project_name} — الوحدة ${u.unit_number}`,
    ...
    <tr class="tot"><td>السعر النهائي</td><td>${money(sale ? sale.final_price : u.price)}</td></tr></tbody></table>`}
