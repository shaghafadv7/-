...
const S = {
  ...
  none: ['—', '#657080'], resold: ['أعيد بيعها', '#315f91'], off_plan: ['تحت الإنشاء', '#a36c1f'],
};
...
function renderSale(p, data) {
  ...
  p.kvGrid([
    ['رقم البيع', s.sale_no], ['تاريخ البيع', s.sale_date], ['رقم العقد', s.contract_no || '—'],
    ['اسم العميل (المالك الجديد)', data.customer.name], ['جوال العميل', data.customer.phone],
  ...
  p.table({
    ...
    rows: [
      ...
      { k: 'المبلغ المدفوع', d: methodAr[s.payment_method] || '—', v: money(s.paid_amount)  },
  ...
  if (fin && (s.is_resale || s.investor_name)) {
    p.space(6);
    p.sectionTitle('المستثمر والأرباح' + (s.investor_name ? ': ' + s.investor_name : ''));
    p.table({
      cols: [
        { label: 'البيان', key: 'k', w: .4 },
      ...
      rows: [
        { k: 'تكلفة الشراء (رأس المال)', v: money(fin.purchase_cost), n: '—' },
        { k: 'سعر البيع النهائي (بعد الخصومات)', v: money(fin.final), n: fin.resale_price != null ? 'سعر إعادة البيع' : '—' },
        { k: 'إجمالي المصروفات', v: money(fin.expenses), n: '—' },
        { k: 'العمولات والرسوم', v: money(fin.commission), n: '—' },
        { k: 'الربح الإجمالي', v: { t: money(fin.gross_profit), c: fin.gross_profit >= 0 ? '#1e6b4f' : '#982d36' }, n: (fin.gross_profit_pct ?? '—') + '%' },
        { k: 'صافي الربح', v: { t: money(fin.net_profit), c: fin.net_profit > 0 ? '#1e6b4f' : (fin.net_profit < 0 ? '#982d36' : '#718096') }, n: (fin.net_profit_pct ?? '—') + '%' },
...
function renderFinancialReport(p, data) {
  ...
  p.bigNumber('عمولات مستحقة', s.commission_due, 'ريال سعودي');
  p.space(4);
...
const RENDERERS = {
  ...
  sale: renderSale, sales_report: renderSalesReport, payments_report: renderPaymentsReport,
  commissions_report: renderCommissionsReport, marketers_report: renderMarketersReport,
  financial_report: renderFinancialReport,
...
async function buildDocument(options) {
  ...
  const p = new ArabicPDF({
    ...
    landscape: ['marketer_report', 'sales_report', 'payments_report', 'commissions_report', 'search_results'].includes(type),
  });
  p.ink = '#1d2a42';
